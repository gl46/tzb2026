"""ADR-0026 terminal diagnostic V3 with a direct-measurement RGB-D setup.

V3 does not change the terminal primitive, public predicates, thresholds, or
safety gates.  It only permits C1/C2 to initialize the same public RGB-D
measurement stack without falsely declaring a legacy M2B failure injection.
The consumed V1/V2 attempts remain immutable and are excluded from V3.
"""
...
from xh_agent.policy.qrm_lite.terminal_regrasp_diagnostic_v2 import (
    ...
    EXCLUSION_PATHS as V2_EXCLUSION_PATHS,
    PARKING_XY_M,
...
CAMPAIGN_ID = "m2c-s4-terminal-regrasp-diagnostic-v3"
PREREG_REPOSITORY_PATH = "docs/decisions/M2C-S4-TERMINAL-REGRASP-DIAGNOSTIC-V3-PREREG.json"
LEDGER_NAMESPACE = "M2C_S4_TERMINAL_REGRASP_DIAGNOSTIC_V3"
V2_PREREG_PATH = "docs/decisions/M2C-S4-TERMINAL-REGRASP-DIAGNOSTIC-V2-PREREG.json"
V2_INCOMPLETE_REPORT_PATH = "reports/m2c-s4-terminal-regrasp-diagnostic-v2-incomplete.json"
DIRECT_PUBLIC_RGBD_CONTRACT = (
    "DIAGNOSTIC_C1_C2_PUBLIC_RGBD_MEASUREMENT_WITHOUT_LEGACY_FAILURE_INJECTION"
)
...
IMPLEMENTATION_PATHS = (
    ...
    "scripts/m2c/build_terminal_regrasp_diagnostic_prereg_v3.py",
    ...
    "scripts/m2c/materialize_terminal_regrasp_diagnostic_scenes_v3.py",
    "scripts/m2c/run_terminal_regrasp_diagnostic.py",
    "scripts/m2c/run_terminal_regrasp_diagnostic_v3.py",
    "scripts/m2c/audit_terminal_regrasp_diagnostic_v3.py",
    "scripts/m2c/audit_terminal_regrasp_diagnostic_v2_incomplete.py",
...
EXCLUSION_PATHS = (*V2_EXCLUSION_PATHS, V2_PREREG_PATH, V2_INCOMPLETE_REPORT_PATH)


class PriorV2InvalidationV3(StrictModel):
    ...
    controller_initialized: Literal[False]
    terminal_primitive_called: Literal[False]
    physical_action_outcome: Literal["NOT_AVAILABLE"]
    v2_claim_reused: Literal[False]
    v2_identity_reused: Literal[False]


class M2CTerminalRegraspDiagnosticPreregV3(StrictModel):
    schema_version: Literal["M2CTerminalRegraspDiagnosticPreregV3"]
    status: Literal["FROZEN_AFTER_V2_PRE_TERMINAL_INVALIDATION_BEFORE_ANY_VALID_DIAGNOSTIC_OUTCOME"]
    ...
    prior_v2_invalidation: PriorV2InvalidationV3
    restart_basis: Literal["NEW_CAMPAIGN_ALL_FRESH_IDENTITIES_NO_V1_OR_V2_CLAIM_OR_IDENTITY_REUSE"]
    ...
    def campaign_is_exact(self) -> "M2CTerminalRegraspDiagnosticPreregV3":
        if self.introduction_commit_paths != [self.repository_relative_path]:
            raise ValueError("V3 diagnostic introduction path is not singleton-exact")
        if self.governing_adr != {
            ...
            raise ValueError("V3 diagnostic governing ADR binding changed")
        if [item.path for item in self.implementation_bindings] != list(IMPLEMENTATION_PATHS):
            raise ValueError("V3 diagnostic implementation closure is not exact")
        if [item.path for item in self.exclusion_sources] != list(EXCLUSION_PATHS):
            raise ValueError("V3 diagnostic exclusion inventory is not exact")
        if self.committed_source_snapshot.commit != self.implementation_commit:
            raise ValueError("V3 diagnostic source snapshot differs from implementation")
        if self.parking_xy_m != {key: list(value) for key, value in PARKING_XY_M.items()}:
            raise ValueError("V3 diagnostic parking geometry changed")
        if [run.ordinal for run in self.runs] != list(range(TOTAL_RUNS)):
            raise ValueError("V3 diagnostic run order is not exact")
        expected_conditions = [
        ...
        if [run.condition for run in self.runs] != expected_conditions:
            raise ValueError("V3 diagnostic paired condition order changed")
        ...
        if len(grouped) != RUNS_PER_CONDITION or any(
            ...
            raise ValueError("V3 diagnostic source-seed pairing changed")
        for field in ("scene_seed", "failure_seed", "run_id", "matched_key"):
            ...
            if len(set(values)) != TOTAL_RUNS:
                raise ValueError(f"V3 diagnostic campaign repeats {field}")
        core = self.model_dump(mode="json", exclude={"prereg_sha256"})
        if self.prereg_sha256 != canonical_sha256(core):
            raise ValueError("V3 diagnostic prereg canonical digest mismatch")
        return self


@dataclass(frozen=True)
class ResolvedTerminalDiagnosticPreregV3:
    path: Path
    file_sha256: str
    raw_bytes: bytes
    prereg: M2CTerminalRegraspDiagnosticPreregV3
    introduced_commit: str


class M2CTerminalDiagnosticConsumptionReceiptV3(StrictModel):
    schema_version: Literal["M2CTerminalDiagnosticConsumptionReceiptV3"]
    ...
    @model_validator(mode="after")
    def receipt_hash_is_exact(self) -> "M2CTerminalDiagnosticConsumptionReceiptV3":
        core = self.model_dump(mode="json", exclude={"receipt_sha256"})
        if self.receipt_sha256 != canonical_sha256(core):
            raise ValueError("V3 diagnostic consumption digest mismatch")
        if self.ordinal != self.run.ordinal:
            raise ValueError("V3 diagnostic consumption ordinal mismatch")
        return self


class M2CTerminalDiagnosticAuthorizationBindingV3(StrictModel):
    schema_version: Literal["M2CTerminalDiagnosticAuthorizationBindingV3"]
    campaign_id: Literal[CAMPAIGN_ID]
    condition: Literal["C1_NO_BLOCKER", "C2_RETAINED_BLOCKER", "C3_FULL_V4"]
    run_id: str
    matched_key: str
    ordinal: int
    scene_seed: int
    ...
    public_rgbd_setup_contract: Literal[DIRECT_PUBLIC_RGBD_CONTRACT]
    ...
    @model_validator(mode="after")
    def direct_mode_matches_condition(self) -> "M2CTerminalDiagnosticAuthorizationBindingV3":
        if self.direct_public_rgbd_enabled is not (
            self.condition in {"C1_NO_BLOCKER", "C2_RETAINED_BLOCKER"}
        ):
            raise ValueError("V3 direct public RGB-D mode differs from condition")
        return self


class M2CTerminalRegraspDiagnosticRawV3(StrictModel):
    schema_version: Literal["M2CTerminalRegraspDiagnosticRawV3"]
    authorization: M2CTerminalDiagnosticAuthorizationBindingV3
    run_id: str
    matched_key: str
    ...
    @model_validator(mode="after")
    def raw_projection_is_exact(self) -> "M2CTerminalRegraspDiagnosticRawV3":
        ...
        if (
            ...
            or self.public_rgbd_setup_contract != auth.public_rgbd_setup_contract
            or self.actuation_probe_source_sha256 != auth.derived_probe_sha256
        ):
            raise ValueError("V3 diagnostic raw identity differs from authorization")
        valid = self.terminal_regrasp_execution is not None
        if self.terminal_measurement_valid is not valid:
            raise ValueError("V3 diagnostic terminal-valid projection mismatch")
        if (self.terminal_execution_status == "NO_TERMINAL_ATTEMPT") is valid:
            raise ValueError("V3 diagnostic terminal status/evidence mismatch")
        success = (
        ...
        if self.terminal_success is not success:
            raise ValueError("V3 diagnostic terminal-success projection mismatch")
        if self.pregrasp_ik_passed is not (
            self.terminal_execution_status
            not in {"PREGRASP_IK_GATE_REJECTED", "NO_TERMINAL_ATTEMPT"}
        ):
            raise ValueError("V3 diagnostic pregrasp projection mismatch")
        if self.contact_gate_passed is not (self.terminal_execution_status == "LIFTED"):
            raise ValueError("V3 diagnostic contact projection mismatch")
        gap = auth.terminal_target_blocker_surface_gap_m
        if gap is None:
            if self.terminal_target_blocker_surface_gap_m is not None:
                raise ValueError("V3 diagnostic C1 raw must have no blocker gap")
        elif self.terminal_target_blocker_surface_gap_m is None or not math.isclose(
            self.terminal_target_blocker_surface_gap_m,
            gap,
            abs_tol=1e-12,
        ):
            raise ValueError("V3 diagnostic raw blocker gap changed")
        return self


def parse_diagnostic_raw_bytes_v3(raw: bytes) -> M2CTerminalRegraspDiagnosticRawV3:
    return M2CTerminalRegraspDiagnosticRawV3.model_validate(
        _json_object(raw, label="V3 terminal diagnostic raw evidence")
    )


def load_committed_diagnostic_prereg_v3(
    *, project_root: Path, prereg_path: Path
) -> ResolvedTerminalDiagnosticPreregV3:
    root = project_root.resolve(strict=True)
    path = prereg_path.resolve(strict=True)
    try:
        relative = path.relative_to(root).as_posix()
    except ValueError as error:
        raise TerminalDiagnosticError("V3 diagnostic prereg escapes project root") from error
    raw = read_regular_file_once(path)
    prereg = M2CTerminalRegraspDiagnosticPreregV3.model_validate(
        _json_object(raw, label="V3 terminal diagnostic prereg")
    )
    if relative != prereg.repository_relative_path:
        raise TerminalDiagnosticError("V3 diagnostic repository path mismatch")
    if _git(root, "show", f"HEAD:{relative}") != raw:
        raise TerminalDiagnosticError("V3 diagnostic prereg is not current HEAD bytes")
    introduced = (
    ...
    if len(introduced) != 1 or _git(root, "show", f"{introduced[0]}:{relative}") != raw:
        raise TerminalDiagnosticError("V3 diagnostic introduction binding changed")
    changed = set(
    ...
    if changed != {relative}:
        raise TerminalDiagnosticError("V3 diagnostic prereg commit is not single-purpose")
    ...
    for binding in prereg.implementation_bindings + prereg.exclusion_sources:
        ...
        if current != committed or sha256_bytes(current) != binding.sha256:
            raise TerminalDiagnosticError(f"V3 source binding changed: {binding.path}")
    ...
    if _git(
        ...
        raise TerminalDiagnosticError("V3 implementation is not a prereg ancestor")
    ...
    if any(not values for values in axes.values()) or (
        ...
        raise TerminalDiagnosticError("V3 external identity inventory changed")
    for run in prereg.runs:
        if any(getattr(run, axis) in values for axis, values in axes.items()):
            raise TerminalDiagnosticError("V3 identity overlaps frozen external inventory")
    return ResolvedTerminalDiagnosticPreregV3(
        path=path,
...
def _read_existing_claims_v3(
    *,
    ledger_root: Path,
    prereg: M2CTerminalRegraspDiagnosticPreregV3,
    expected_count: int,
) -> list[M2CTerminalDiagnosticConsumptionReceiptV3]:
    ...
    if names != expected:
        raise TerminalDiagnosticError("V3 diagnostic claims are not in frozen order")
    claims: list[M2CTerminalDiagnosticConsumptionReceiptV3] = []
    for ordinal, name in enumerate(names):
        claim = M2CTerminalDiagnosticConsumptionReceiptV3.model_validate(
            _json_object(
                read_regular_file_once(ledger_root / name),
                label=f"V3 diagnostic claim {name}",
            )
        )
        if claim.ordinal != ordinal or claim.run != prereg.runs[ordinal]:
            raise TerminalDiagnosticError("V3 diagnostic ledger prefix changed")
        claims.append(claim)
    return claims


def consume_diagnostic_run_v3(
    *,
    resolved: ResolvedTerminalDiagnosticPreregV3,
    run_id: str,
    ledger_root: Path,
    ...
    if str(ledger_root.resolve(strict=True)) != prereg.ledger_root:
        raise TerminalDiagnosticError("V3 diagnostic ledger root differs from prereg")
    _secure_ledger_root(ledger_root)
    selected = [run for run in prereg.runs if run.run_id == run_id]
    if len(selected) != 1:
        raise TerminalDiagnosticError("V3 diagnostic run is not uniquely preregistered")
    run = selected[0]
    with _ledger_lock(ledger_root):
        _read_existing_claims_v3(
            ledger_root=ledger_root,
            prereg=prereg,
            expected_count=run.ordinal,
        )
        payload: dict[str, Any] = {
            "schema_version": "M2CTerminalDiagnosticConsumptionReceiptV3",
        ...
        receipt = M2CTerminalDiagnosticConsumptionReceiptV3.model_validate(payload)
...
def bind_diagnostic_claim_to_raw_session_v3(
    *,
    claim_path: Path,
    ...
    claim = M2CTerminalDiagnosticConsumptionReceiptV3.model_validate(
        _json_object(read_regular_file_once(claim_path), label="V3 diagnostic claim")
    )
    run = claim.run
    expected = (
        run.run_id,
    ...
    if actual != expected:
        raise TerminalDiagnosticError("V3 probe inputs differ from consumed claim")
    verify_materialized_source_snapshot(
    ...
    return M2CTerminalDiagnosticAuthorizationBindingV3(
...
def evaluate_preregistered_decision_tree_v3(
    *,
    prereg: M2CTerminalRegraspDiagnosticPreregV3,
...
__all__ = [
    ...
    "V2_PREREG_PATH",
    "bind_diagnostic_claim_to_raw_session_v3",
    "consume_diagnostic_run_v3",
    "evaluate_preregistered_decision_tree_v3",
