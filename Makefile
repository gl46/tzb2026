...
m2c-freeze:
	M2B_EVIDENCE_READONLY=1 $(PYTHON) scripts/m2c/freeze_m2b.py
m2c-s1:
	M2B_EVIDENCE_READONLY=1 $(PYTHON) scripts/m2c/export_s1_evidence.py \
		--host "$${ISAAC_HOST:-root@labserver}" \
		--output-root "$${M2C_EVIDENCE_ROOT:-/Users/gl/tzb-m2c-evidence}/m2c-s1"
m2c-s2-domain:
	M2B_EVIDENCE_READONLY=1 $(PYTHON) scripts/m2c/build_headroom_domain.py \
		--output-root "$${M2C_EVIDENCE_ROOT:-/Users/gl/tzb-m2c-evidence}/m2c-s2/source" \
		--manifest configs/m2c_headroom_domain.json
m2c-s2-domain-v2:
	M2B_EVIDENCE_READONLY=1 $(PYTHON) scripts/m2c/build_tall_headroom_domain.py \
		--output-root "$${M2C_EVIDENCE_ROOT:-/Users/gl/tzb-m2c-evidence}/m2c-s2/source-v2" \
		--manifest configs/m2c_headroom_domain_v2.json
m2c-s2-domain-v3:
	M2B_EVIDENCE_READONLY=1 $(PYTHON) scripts/m2c/build_blocked_headroom_domain.py \
		--output-root "$${M2C_EVIDENCE_ROOT:-/Users/gl/tzb-m2c-evidence}/m2c-s2/source-v3" \
		--manifest configs/m2c_headroom_domain_v3.json
m2c-s2-domain-v4:
	M2B_EVIDENCE_READONLY=1 $(PYTHON) scripts/m2c/build_final_headroom_domain.py \
		--output-root "$${M2C_EVIDENCE_ROOT:-/Users/gl/tzb-m2c-evidence}/m2c-s2/source-v4" \
		--manifest configs/m2c_headroom_domain_v4.json
m2c-s3-source:
	M2B_EVIDENCE_READONLY=1 $(PYTHON) scripts/m2c/build_s3_collection_plan.py \
		--output-root "$${M2C_EVIDENCE_ROOT:-/Users/gl/tzb-m2c-evidence}/m2c-s3/source-v1" \
		--manifest configs/m2c_s3_collection_plan.json
m2c-s3-evidence-freeze:
	M2B_EVIDENCE_READONLY=1 $(PYTHON) scripts/m2c/freeze_s3_evidence.py \
		--report reports/m2c-s3-evidence-freeze.json
m2c-s3-dataset: m2c-s3-evidence-freeze
	M2B_EVIDENCE_READONLY=1 $(PYTHON) scripts/m2c/build_s3_dataset.py \
		--freeze-report reports/m2c-s3-evidence-freeze.json \
		--worker-status /var/tmp/xh-data/isaac-industrial/m2c/s3-failure-evidence-v1/worker0/worker-status.json \
		--worker-status /var/tmp/xh-data/isaac-industrial/m2c/s3-failure-evidence-v1/worker1/worker-status.json \
...
# Evidence-only: this target never trains a residual or starts a physical run.
m2c-s5:
	@test -n "$${M2C_S5_EPISODES:-}" || { echo "M2C_S5_EPISODES is required" >&2; exit 2; }
	@test -n "$${M2C_S5_EVIDENCE_ROOT:-}" || { echo "M2C_S5_EVIDENCE_ROOT is required" >&2; exit 2; }
	@test -n "$${M2C_S5_R6D_DISPOSITION:-}" || { echo "M2C_S5_R6D_DISPOSITION is required" >&2; exit 2; }
	@test -n "$${M2C_S5_GRIPPER_DISPOSITION:-}" || { echo "M2C_S5_GRIPPER_DISPOSITION is required" >&2; exit 2; }
	@test -n "$${M2C_S5_GOVERNANCE_RECEIPT:-}" || { echo "M2C_S5_GOVERNANCE_RECEIPT is required; the NOT APPROVED request is not valid" >&2; exit 2; }
	M2B_EVIDENCE_READONLY=1 $(PYTHON) scripts/m2c/summarize_s5_residual_closed_loop.py \
		--episodes "$${M2C_S5_EPISODES}" \
		--evidence-root "$${M2C_S5_EVIDENCE_ROOT}" \
		--r6d-disposition "$${M2C_S5_R6D_DISPOSITION}" \
		--gripper-disposition "$${M2C_S5_GRIPPER_DISPOSITION}" \
		--governance-receipt "$${M2C_S5_GOVERNANCE_RECEIPT}" \
		--report reports/m2c-s5-residual-closed-loop.json

# Evidence-only: this target never starts Isaac, training, inference, or a remote job.
m2c-s6:
	@test -n "$${M2C_S6_EPISODES:-}" || { echo "M2C_S6_EPISODES is required" >&2; exit 2; }
	@test -n "$${M2C_S6_EVIDENCE_MANIFEST:-}" || { echo "M2C_S6_EVIDENCE_MANIFEST is required" >&2; exit 2; }
	@test -n "$${M2C_S6_PHYSICAL_RECEIPTS_ROOT:-}" || { echo "M2C_S6_PHYSICAL_RECEIPTS_ROOT is required" >&2; exit 2; }
	M2B_EVIDENCE_READONLY=1 $(PYTHON) scripts/m2c/summarize_s6_matched.py \
		--episodes "$${M2C_S6_EPISODES}" \
		--evidence-manifest "$${M2C_S6_EVIDENCE_MANIFEST}" \
		--physical-receipts-root "$${M2C_S6_PHYSICAL_RECEIPTS_ROOT}" \
		--frozen-keys configs/m2c_s6_evaluation_keys.json \
		--preregistration docs/decisions/M2C-S6-MATCHED-EVALUATION-PREREG.md \
		--report reports/m2c-s6-matched.json \
		--report-md reports/m2c-s6-matched.md
