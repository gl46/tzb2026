...
| 9 | M1A cube regression | **BLOCKED — dirty tree** (`BLOCKED_DIRTY_OR_MOVED_BASELINE`): the preflight refuses to run the cube regression on an uncommitted working tree. Runs once the ADR-0016b work is committed. |
| — | Unit suite (acceptance item 6) | **110/110 pass** with the ADR-0016b hand contract; `validate_project` PASS; changed scripts lint clean. |
