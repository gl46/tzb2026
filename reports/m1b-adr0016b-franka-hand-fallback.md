...
| 6b | Z-envelope re-derivation | **root cause proven; production centreline 120→118 mm** (`b894c58`, `reports/m1b-adr0016b-z-envelope-geometry.md`). The 5 mm Z envelope was not grasp noise but two hard geometric bounds with opposite sensitivity to the contact centreline: the palm box bottom striking the cylinder top face (`δ ≥ 0.106 − H`, presenting as collision-aware descent-IK rejection) and the plate tip riding above the measured +17.8 mm tipping height (`δ ≤ 0.130 − H`, presenting as single-sided contact). The model reproduces every campaign Z result exactly. Confirmed: at H=130 mm the δ=−15 mm point flips **0/3 → 3/3** with attach. The bounds meet at **118 mm**; pre-check there holds −10 mm at 3/3 and improves +10 mm from **1/3 → 2/3**, doubling the measurable envelope and taking the Z gate margin 0.92 → ~3.9 mm. 81-trial revalidation at 118 mm in progress. |
| 7 | Perception p90 gate | **NO_GO at the time of this run; since resolved on both sides.** This run measured p90 9.88/15.13/17.30 mm against 9/9/3 mm limits. The perception side turned out to be a capture-contamination artifact, not sensor accuracy: isolating each RGB-D capture world/process group and re-gating centres on public geometry brought current-geometry held-out p90 to **2.295/4.793/2.080 mm**, and the gate to **GO** (`reports/m1b-adr0016b-current-geometry-reachability-gate.json`). The envelope side then bound it: Z passed by only **0.92 mm**, which is what made individual grasps unreliable even with a GO gate. Root-caused and fixed in step 6b below. Full history in `reports/m1b-adr0016b-perception-gate-analysis.md` (+ addendum). |
...
| 9 | M1A cube regression | **RUN (clean worktree) — surfaced a pre-existing subsystem bug (bounded follow-up, out of fallback scope).** Under the new hand: bullet audit 5/5, home, S0 13/13, S1 10/10 pass. The M1A cube contact-gated grasp fails; deep root-cause: `src/xh_agent/grasp/orientation_families.py` builds grasp poses on the **pre-ADR-0016 local-+X finger-axis convention** (`test_..._keeps_the_end_effector...` line 517: `Rot·(1,0,0) == (0,0,−1)`, i.e. local X = the down-pointing finger axis), while the hand's fingers have run along **local +Z** since ADR-0016 (URDF finger joints `origin 0 0 0.0584`, extent +z). The anchor constants (`FINGER_LENGTH_M=0.12`, `FINGER_ROOT_Z_M=0.055`, `HAND_FINGER_LINE_X_M=0.06`) match the old +X finger reach, so a live corridor probe places the cube at panda_hand-frame **(x 0.105, y 0.0006, z 0.055)** — off the franka plate (x ∈ [−0.0105, 0.0105], z-centre 0.027) along the wrong axis. So the contact-gated grasp has been geometrically wrong since ADR-0016, independent of the franka fallback (the fallback only changed the exact pad box). **The re-tuned 0.027 close (committed) is S0-aligned but cannot help until the pose is fixed. Follow-up: rebuild orientation_families for the +Z finger axis (anchor constants + candidate families + unit tests) and re-validate — a separate subsystem re-derive, not a hand-topology change.** S0's own vertical-offset bilateral pose (updated for +Z) already contacts the cube, so the fixture and pad geometry are sound. Worktree M1A reports NOT committed. |
| — | Unit suite (acceptance item 6) | **111/111 pass** with the ADR-0016b hand contract + §3 corridor prefilter; `validate_project` PASS; changed scripts lint clean. Committed `ca39515`..`f21e0c2`. |
