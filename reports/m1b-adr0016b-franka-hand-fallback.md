...
| 8 | ADR-0013 acceptance 4 (wrong-object drill) | **NOT RUN live** — the WRONG_OBJECT identity logic and recovery routing are unit-verified (`test_post_grasp_identity_routes_wrong_object_without_entity_leak`) and the broker's same-entity honesty (no target-id gate) is proven by the round-trip; the full perception-in-the-loop live drill remains. |
| 9 | M1A cube regression | **RUN (clean worktree at committed HEAD) — partial: one real regression found.** Under the new hand: bullet audit 5/5 (incl. cube_table_stability + detachable_round_trip), home self-collision VERIFIED, S0 13/13, S1 10/10 all pass. **But the M1A cube contact-gated constrained grasp fails 10/10** (`primary_failure_class = CONTACT_CLOSURE_FAILURE`, gate `LEFT_FINGER_TARGET_CONTACT_MISSING`; evidence SHA-256 `06a6d0a53655…`). Root cause is a geometry re-tune the new hand needs: the cube pick's `close_command_per_finger_m = 0.033` gives a 66 mm inner width, and with the franka pad face modelled 6.5 mm proud it leaves ~1.5 mm gap per side on the 50 mm cube — no finger–target contact (the cube-side sensor still logs 256 events; the finger sensors log 0). The S0 bilateral command that *does* contact the same cube is 0.028/finger. b1-oracle is blocked downstream. **Follow-up: re-tune the M1A cube contact-gated grasp close (and likely a two-stage close, as M1B needed) for the franka pad geometry — a second grasp primitive, separate from the M1B top-down family tuned here.** The regenerated worktree M1A reports were NOT committed (they belong to a separate M1A-under-new-hand re-baseline). |
| — | Unit suite (acceptance item 6) | **110/110 pass** with the ADR-0016b hand contract; `validate_project` PASS; changed scripts lint clean. Committed as `ca39515`. |
