...
- Checked parent HEAD: `7066f74acda8a0308d03b287a419652ecf57842d`
- Q-A: **PASSED**
- Q-B: **UNMEASURED**
...
Across the fourteen immutable V3/V4 collection reports, 43 distinct TRAIN identities produced 34 complete eight-step chains and zero eligible episodes. The updated ADR-0025 section-3 audit measures both per-key yield (0/43) and complete-chain conditional yield (0/34) as 0.0. At that observed point yield there is no finite evidence-based projection even for the trainer's code minimum of one eligible episode. This blocks training; it does not claim model capability is zero. Model rollout and formal Q-B evaluation have not run, so pure success remains `null`.

Ten complete V3 chains and the twenty-three Batch-09/10/11/12/13/14/15/16 V4 chains passed gates for steps 0–6. Across all complete chains, twenty-three terminate at the contact/controller gate, ten at pregrasp IK, and one lifts but fails the public success predicate. This supports a recurring terminal regrasp approach/contact-acceptance mismatch but does not isolate perception offset, approach geometry, or object state. No gate, threshold, B0 byte, or frozen eligibility predicate was changed.

Batch-16 was preregistered outcome-blind and all three selected keys were consumed exactly once. All three stage/probe chains completed and produced 24 physical receipts with zero safety violations. Scene 19267 remained ineligible at the unchanged REGRASP pregrasp-IK gate; scenes 19285 and 19291 remained ineligible at the unchanged contact gate. Scene 19291 also failed the offline step-2 and step-4 public-target-in-K8 conditions. No subsequent preregistration is active in this snapshot, so collection is fail-closed again.
...
```bash
PYTHONPATH=src:scripts uv run pytest -q tests/unit/test_m2c_s4_current_blockers_report.py tests/unit/test_m2c_s4_v4_batch16_collection_audit.py
```
