...
- Checked parent HEAD: `f4df0bf99ac9c118bac9671938fc9e6ff03c6423`
- Q-A: **PASSED**
- Q-B: **UNMEASURED**
...
Across the seventeen immutable V3/V4 collection reports, 50 distinct TRAIN identities produced 41 complete eight-step chains and zero eligible episodes. The updated ADR-0025 section-3 audit measures both per-key yield (0/50) and complete-chain conditional yield (0/41) as 0.0. At that observed point yield there is no finite evidence-based projection even for the trainer's code minimum of one eligible episode. This blocks training; it does not claim model capability is zero. Model rollout and formal Q-B evaluation have not run, so pure success remains `null`.

Ten complete V3 chains and the thirty Batch-09 through Batch-19 V4 chains passed gates for steps 0–6. Across all complete chains, twenty-seven terminate at the contact/controller gate, thirteen at pregrasp IK, and one lifts but fails the public success predicate. This supports a recurring terminal regrasp approach/contact-acceptance mismatch but does not isolate perception offset, approach geometry, or object state. No gate, threshold, B0 byte, or frozen eligibility predicate was changed.

Batch-19 preregistered the first three identity-disjoint V4 extension keys outcome-blind and consumed each exactly once. All three completed stage/probe chains and produced 24 physical receipts with zero safety violations; scenes 22001 and 22002 remained ineligible at the unchanged REGRASP contact gate, and scene 22007 remained ineligible at the unchanged pregrasp-IK gate. The original V4 TRAIN manifest remains exhausted at 36/36; extension1 has consumed 3/36 with 33 unconsumed. No subsequent preregistration is active, so collection is fail-closed.
...
```bash
PYTHONPATH=src:scripts uv run pytest -q tests/unit/test_m2c_s4_current_blockers_report.py tests/unit/test_m2c_s4_v4_batch19_collection_audit.py
```
