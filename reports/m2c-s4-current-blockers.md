...
- Checked parent HEAD: `a2b6097a7f618dd9b923dcc0220a2600e927ca92`
- Q-A: **PASSED**
- Q-B: **UNMEASURED**
...
Across the twelve immutable V3/V4 collection reports, 37 distinct TRAIN identities produced 28 complete eight-step chains and zero eligible episodes. The updated ADR-0025 section-3 audit measures both per-key yield (0/37) and complete-chain conditional yield (0/28) as 0.0. At that observed point yield there is no finite evidence-based projection even for the trainer's code minimum of one eligible episode. This blocks training; it does not claim model capability is zero. Model rollout and formal Q-B evaluation have not run, so pure success remains `null`.

Ten complete V3 chains and the seventeen Batch-09/10/11/12/13/14 V4 chains passed gates for steps 0–6. Across all complete chains, nineteen terminate at the contact/controller gate, eight at pregrasp IK, and one lifts but fails the public success predicate. This supports a recurring terminal regrasp approach/contact-acceptance mismatch but does not isolate perception offset, approach geometry, or object state. No gate, threshold, B0 byte, or frozen eligibility predicate was changed.

Batch-14 was preregistered outcome-blind and all three selected keys were consumed exactly once. All three stage/probe chains completed and produced 24 physical receipts with zero safety violations. Scene 19227 remained ineligible at the unchanged REGRASP pregrasp-IK gate; scenes 19229 and 19231 remained ineligible at the unchanged contact gate. Scene 19231 also failed the offline step-1 public-target-in-K8 condition. No subsequent preregistration is active in this snapshot, so collection is fail-closed again.
...
```bash
PYTHONPATH=src:scripts uv run pytest -q tests/unit/test_m2c_s4_current_blockers_report.py tests/unit/test_m2c_s4_v4_batch14_collection_audit.py
```
