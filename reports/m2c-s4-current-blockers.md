...
- Checked parent HEAD: `608e2ff5074785a837b9ccaccfa444df518f8f92`
- Q-A: **PASSED**
- Q-B: **UNMEASURED**
...
Across the thirteen immutable V3/V4 collection reports, 40 distinct TRAIN identities produced 31 complete eight-step chains and zero eligible episodes. The updated ADR-0025 section-3 audit measures both per-key yield (0/40) and complete-chain conditional yield (0/31) as 0.0. At that observed point yield there is no finite evidence-based projection even for the trainer's code minimum of one eligible episode. This blocks training; it does not claim model capability is zero. Model rollout and formal Q-B evaluation have not run, so pure success remains `null`.

Ten complete V3 chains and the twenty Batch-09/10/11/12/13/14/15 V4 chains passed gates for steps 0–6. Across all complete chains, twenty-one terminate at the contact/controller gate, nine at pregrasp IK, and one lifts but fails the public success predicate. This supports a recurring terminal regrasp approach/contact-acceptance mismatch but does not isolate perception offset, approach geometry, or object state. No gate, threshold, B0 byte, or frozen eligibility predicate was changed.

Batch-15 was preregistered outcome-blind and all three selected keys were consumed exactly once. All three stage/probe chains completed and produced 24 physical receipts with zero safety violations. Scene 19233 remained ineligible at the unchanged REGRASP pregrasp-IK gate; scenes 19244 and 19265 remained ineligible at the unchanged contact gate. Scene 19265 also failed the offline step-1 public-target-in-K8 condition. No subsequent preregistration is active in this snapshot, so collection is fail-closed again.
...
```bash
PYTHONPATH=src:scripts uv run pytest -q tests/unit/test_m2c_s4_current_blockers_report.py tests/unit/test_m2c_s4_v4_batch15_collection_audit.py
```
