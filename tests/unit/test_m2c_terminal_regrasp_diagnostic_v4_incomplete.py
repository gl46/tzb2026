...
from m2c.audit_terminal_regrasp_diagnostic_v4_incomplete import (
    CRASH_FRAGMENTS,
    audit_v4_incomplete_campaign,
)
...
PREREG = ROOT / "docs/decisions/M2C-S4-TERMINAL-REGRASP-DIAGNOSTIC-V4-PREREG.json"
...
def _fixture(tmp_path: Path) -> tuple[Path, Path]:
    ...
    run = prereg.runs[0]
    derived = derive_terminal_regrasp_diagnostic_probe_bytes_v4(UPSTREAM.read_bytes())
    claim: dict[str, object] = {
        ...
        "direct_public_rgbd_setup_contract": prereg.direct_public_rgbd_setup_contract,
        "consumed_at_ns": 100,
        "teacher_used": False,
    ...
    _write(job_root / "derived" / "terminal-regrasp-diagnostic-probe-v4.py", derived)
    ...
    _write(
        job_root / "stage" / "console.log",
        ("\n".join(CRASH_FRAGMENTS) + "\n").encode(),
    )
    _write(job_root / "stage" / "m1b_physics_scene.usdc", b"partial-stage")
    (job_root / "probe").mkdir(parents=True)
    return ledger, evidence


def test_v4_incomplete_audit_proves_stage_crash_before_probe(tmp_path: Path) -> None:
    ...
    assert report["status"] == "BLOCKED_INCOMPLETE_STAGE_SIGSEGV_BEFORE_DIAGNOSTIC_PROBE"
    assert report["valid_terminal_measurements"] == 0
    assert report["decision"] is None
    assert report["failed_run"]["stage_kit_started"] is True
    assert report["failed_run"]["diagnostic_probe_started"] is False
    assert report["failed_run"]["terminal_primitive_called"] is False
    assert report["further_v4_execution_authorized"] is False


def test_v4_incomplete_audit_rejects_missing_crash_signature(tmp_path: Path) -> None:
    ledger, evidence = _fixture(tmp_path)
    job = next(evidence.glob("run-*"))
    (job / "stage" / "console.log").write_text("ordinary stage failure\n")
    with pytest.raises(TerminalDiagnosticError, match="does not prove"):
        audit_v4_incomplete_campaign(
            project_root=ROOT,
            prereg_path=PREREG,
            ledger_root=ledger,
...
def test_v4_incomplete_audit_rejects_probe_evidence(tmp_path: Path) -> None:
    ledger, evidence = _fixture(tmp_path)
    job = next(evidence.glob("run-*"))
    _write(job / "probe" / "console.log", b"probe started\n")
    with pytest.raises(TerminalDiagnosticError, match="unexpectedly reached"):
        audit_v4_incomplete_campaign(
            project_root=ROOT,
            prereg_path=PREREG,
            ledger_root=ledger,
...
def test_published_v4_incomplete_report_withholds_ablation_decision() -> None:
    report = json.loads(
        (ROOT / "reports/m2c-s4-terminal-regrasp-diagnostic-v4-incomplete.json").read_bytes()
    )
    assert report["status"] == "BLOCKED_INCOMPLETE_STAGE_SIGSEGV_BEFORE_DIAGNOSTIC_PROBE"
    assert report["valid_terminal_measurements"] == 0
    assert report["decision"] is None
    assert report["failed_run"]["stage_completed"] is False
    assert report["failed_run"]["diagnostic_probe_started"] is False
