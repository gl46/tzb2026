...
def test_current_s4_blocker_report_replays_v3_and_v4_collection() -> None:
    ...
    assert _v4_counts(v4_bindings) == (14, 7, 56, 0)
    assert training["unique_v3_train_keys_attempted"] == 11
    assert training["unique_v4_train_keys_consumed"] == 14
    assert training["unique_train_keys_consumed_total"] == 25
    assert training["raw_v3_eight_step_chains"] == 10
    assert training["raw_v4_eight_step_chains"] == 7
    assert training["physical_skill_receipts_v4"] == 56
...
def test_raw_detection_capacity_is_accepted_but_training_remains_fail_closed() -> None:
    ...
    batch08 = _load_binding(training["source_reports"][-3])
    raw_attempt = next(
    ...
    batch09 = _load_binding(training["source_reports"][-2])
    ...
    batch10 = _load_binding(training["source_reports"][-1])
