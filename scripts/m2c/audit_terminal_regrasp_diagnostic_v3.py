#!/usr/bin/env python3
"""Replay ADR-0026 V3 measurements and apply the frozen decision tree."""
...
def _verify_terminal_hash(terminal: Mapping[str, Any]) -> None:
    ...
    if not isinstance(claimed, str):
        raise TerminalDiagnosticError("V3 terminal receipt lacks semantic hash")
    core = dict(terminal)
    del core["terminal_receipt_sha256"]
    if canonical_sha256(core) != claimed:
        raise TerminalDiagnosticError("V3 terminal receipt semantic hash changed")


def audit_campaign_v3(
    *,
    project_root: Path,
    prereg_path: Path,
    ledger_root: Path,
    ...
    for run in resolved.prereg.runs:
        ...
        try:
            ...
            if claim.run != run or claim.prereg_sha256 != resolved.prereg.prereg_sha256:
                raise TerminalDiagnosticError("V3 claim differs from frozen run")
            terminal_raw = v4_auth.read_regular_file_once(terminal_path)
            terminal = _json_object(terminal_raw, label="V3 terminal receipt")
            _verify_terminal_hash(terminal)
            raw_bytes = v4_auth.read_regular_file_once(raw_path)
            raw = parse_diagnostic_raw_bytes_v3(raw_bytes)
            if (
                terminal.get("schema_version") != "M2CTerminalDiagnosticRunTerminalV3"
                ...
                raise TerminalDiagnosticError("V3 terminal/raw/claim binding changed")
    ...
    decision = None
    if not blockers:
        decision = evaluate_preregistered_decision_tree_v3(
    ...
    report: dict[str, Any] = {
        "schema_version": "M2CTerminalRegraspDiagnosticAuditV3",
        ...
        "prior_v2_invalidation": resolved.prereg.prior_v2_invalidation.model_dump(mode="json"),
        ...
        "no_retry_or_replacement_within_v3": True,
        "v1_or_v2_claim_or_identity_reused": False,
...
def _markdown(report: Mapping[str, Any]) -> str:
    lines = [
        "# M2C terminal-regrasp diagnostic V3 audit",
        "",
        ...
        "- V1/V2 claim or identity reused: `false`",
        "- Retry/replacement within V3: `false`",
