#!/usr/bin/env python3
"""Replay all three consumed Batch-21 attempts without upgrading evidence."""
...
PREREG_RELATIVE = Path("docs/decisions/M2C-S4-V4-TRAIN-COLLECTION-BATCH-21-PREREG.json")
PREREG_FILE_SHA256 = "303a05d0f4d9a77450f042820d945e1a2e8c6fb1ee0fc6897e9c36138026065d"
PREREG_SHA256 = "0c9c661dfaba6c9597b61a16ef0a3ea99bfe56378f27d2270f642ba9893e4043"
PREREG_COMMIT = "250a89df41fa43eace1ffd9e53d374b4472d8752"
SOURCE_SNAPSHOT_COMMIT = "722b6be309cc7081b8631f2590d7349ae404d032"
SOURCE_SNAPSHOT_INVENTORY_SHA256 = (
    "01a87f4341dc16bfcd5b72f28fd42da9b6a17a8f50e8371f9b7f61c9fe1f0a52"
)
PREDECESSOR_RECEIPT_SHA256 = "89a38a6c35c3b58c05a810ac725106693a1027715c3b54545e4cdaa6f81e5b99"
CLAIM_FILE_SHA256 = (
    "4f81d8f60407eac7c5d7073fe41911f37e6d49636c353d7b8a364373abf82af8",
    "ad0b0e75a4aff1e18b16f1a975437458a8e38a1d3e195bb0f779a19888d37dc8",
    "aa05e4a857480fb7c65261ece539ba343324ad2138f5886d741a11022e67a736",
)
...
class Batch21AuditError(Batch10AuditError):
    """Batch-21 bytes do not prove the bounded zero-eligible outcome."""
...
def _validate_claim_and_job(
    *,
    ...
    try:
        ...
        claim = json_object(claim_raw, label=f"Batch-21 claim {ordinal}")
        job = json_object(read_regular_file_once(root / relative["job"]), label="Batch-21 job")
    except FileNotFoundError as error:
        raise Batch21AuditError("Batch-21 claim or job evidence is missing") from error
    if sha256_bytes(claim_raw) != CLAIM_FILE_SHA256[ordinal] or claim_raw != projected_raw:
        raise Batch21AuditError("Batch-21 canonical and projected claim bytes differ")
    if not _claim_core_is_valid(claim):
        raise Batch21AuditError("Batch-21 claim semantic receipt hash is invalid")
    if (
        claim.get("schema_version") != "M2CS4V4CollectionConsumptionReceiptV1"
        or claim.get("batch_id") != "m2c-s4-v4-train-batch-21"
        or claim.get("ledger_namespace") != "M2C_S4_V4_COLLECTION_BATCH21"
        ...
        raise Batch21AuditError("Batch-21 claim identity or hash chain differs")
    ...
    if (
        ...
        raise Batch21AuditError("Batch-21 job does not bind the consumed claim")
    if inventory.get(relative["derived_probe"]) != claim.get("derived_probe_sha256"):
        raise Batch21AuditError("Batch-21 derived probe hash differs from its claim")
...
def build_report(*, evidence_root: Path, project_root: Path = ROOT) -> dict[str, Any]:
    ...
    if sha256_bytes(prereg_raw) != PREREG_FILE_SHA256:
        raise Batch21AuditError("Batch-21 preregistration bytes changed")
    if (
        _git_blob(project_root=project_root, commit=PREREG_COMMIT, relative_path=PREREG_RELATIVE)
        != prereg_raw
    ):
        raise Batch21AuditError("Batch-21 preregistration differs from its introduction blob")
    prereg = json_object(prereg_raw, label="Batch-21 preregistration")
    ...
    if (
        ...
        raise Batch21AuditError("Batch-21 preregistration contract differs")
    inventory = _inventory(root)
    if len(inventory) != 168:
        raise Batch21AuditError("Batch-21 evidence inventory count differs")
    ...
    if job_keys != {str(item["matched_key"]) for item in selected}:
        raise Batch21AuditError("Batch-21 evidence job set differs from selected keys")
    ...
    for ordinal, identity in enumerate(selected):
        if not isinstance(identity, Mapping):
            raise Batch21AuditError("Batch-21 selected identity is malformed")
        ...
        if len(job_inventory) != 55:
            raise Batch21AuditError("Batch-21 per-job evidence inventory count differs")
        try:
            ...
            raise Batch21AuditError(str(error)) from error
        if (
            tuple(physical["offline_dataset_exclusion_reasons"])
            != EXPECTED_DATASET_EXCLUSIONS[ordinal]
        ):
            raise Batch21AuditError("Batch-21 offline dataset exclusion reasons differ")
        raw_probe = json_object(
            read_regular_file_once(root / relative["raw_probe"]), label="Batch-21 raw probe"
        )
        current_receipt_ids = {
        ...
        if all_receipt_ids & current_receipt_ids:
            raise Batch21AuditError("Batch-21 physical receipt ID is reused")
        all_receipt_ids.update(current_receipt_ids)
        attempts.append(
            {
                "attempt_id": f"batch21-{ordinal + 1:02d}",
                "identity": identity,
    ...
    if (root / "packaged").exists() and any((root / "packaged").rglob("*")):
        raise Batch21AuditError("Batch-21 evidence contains packaged output")
    ...
    return {
        ...
        "audit_implementation": {
            "path": "scripts/m2c/audit_s4_v4_batch21_collection.py",
...
def render_markdown(report: Mapping[str, Any]) -> str:
    return f"""# M2C S4 V4 Batch-21 collection audit
...
def main() -> int:
    ...
    if args.expected_json is not None and read_regular_file_once(args.expected_json) != encoded:
        raise SystemExit("Batch-21 audit differs from expected JSON")
    if args.markdown:
