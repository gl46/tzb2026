#!/usr/bin/env python3
"""Replay all three consumed Batch-17 attempts without upgrading evidence."""
...
PREREG_RELATIVE = Path("docs/decisions/M2C-S4-V4-TRAIN-COLLECTION-BATCH-17-PREREG.json")
PREREG_FILE_SHA256 = "da4ebbce2b188d77fb72204b153585305ced286ef4c5725c4fc449bbc35dc2f7"
PREREG_SHA256 = "f0019127934a1335e113e9e21eeaf4b822a6c72f5fc42c83499dc24a295d9ed6"
PREREG_COMMIT = "ec127cb7977e11ddb4352243dfe56d4f113dbf3a"
SOURCE_SNAPSHOT_COMMIT = "bbe0d8e72625782edb992fba715de44a111a3a88"
SOURCE_SNAPSHOT_INVENTORY_SHA256 = (
    "64406621b1d718490935e6b0758c688b4e4510922dd7f1d35b8addc8bcc0e3b6"
)
PREDECESSOR_RECEIPT_SHA256 = "5863fb6911ef6bf0629f6a4bd14b80bb53c19a44f236debfb6b49606a48e0336"
CLAIM_FILE_SHA256 = (
    "a276b7cae5d5d00bbd7a40e45d3cdf2792700711d2d692ff5f4840997ee300b2",
    "57d8c0ac91b74ef0b4b89139df57b5d4bc27adcc6719956649c5a3e895c45b2e",
    "c344f5b430052a22fd61932c993e43ebd82e421d57e808be327a3ebc695caec0",
)
...
class Batch17AuditError(Batch10AuditError):
    """Batch-17 bytes do not prove the bounded zero-eligible outcome."""
...
def _validate_claim_and_job(
    *,
    ...
    try:
        ...
        claim = json_object(claim_raw, label=f"Batch-17 claim {ordinal}")
        job = json_object(read_regular_file_once(root / relative["job"]), label="Batch-17 job")
    except FileNotFoundError as error:
        raise Batch17AuditError("Batch-17 claim or job evidence is missing") from error
    if sha256_bytes(claim_raw) != CLAIM_FILE_SHA256[ordinal] or claim_raw != projected_raw:
        raise Batch17AuditError("Batch-17 canonical and projected claim bytes differ")
    if not _claim_core_is_valid(claim):
        raise Batch17AuditError("Batch-17 claim semantic receipt hash is invalid")
    if (
        claim.get("schema_version") != "M2CS4V4CollectionConsumptionReceiptV1"
        or claim.get("batch_id") != "m2c-s4-v4-train-batch-17"
        or claim.get("ledger_namespace") != "M2C_S4_V4_COLLECTION_BATCH17"
        ...
        raise Batch17AuditError("Batch-17 claim identity or hash chain differs")
    ...
    if (
        ...
        raise Batch17AuditError("Batch-17 job does not bind the consumed claim")
    if inventory.get(relative["derived_probe"]) != claim.get("derived_probe_sha256"):
        raise Batch17AuditError("Batch-17 derived probe hash differs from its claim")
...
def build_report(*, evidence_root: Path, project_root: Path = ROOT) -> dict[str, Any]:
    ...
    if sha256_bytes(prereg_raw) != PREREG_FILE_SHA256:
        raise Batch17AuditError("Batch-17 preregistration bytes changed")
    if (
        _git_blob(project_root=project_root, commit=PREREG_COMMIT, relative_path=PREREG_RELATIVE)
        != prereg_raw
    ):
        raise Batch17AuditError("Batch-17 preregistration differs from its introduction blob")
    prereg = json_object(prereg_raw, label="Batch-17 preregistration")
    ...
    if (
        ...
        raise Batch17AuditError("Batch-17 preregistration contract differs")
    inventory = _inventory(root)
    if len(inventory) != 168:
        raise Batch17AuditError("Batch-17 evidence inventory count differs")
    ...
    if job_keys != {str(item["matched_key"]) for item in selected}:
        raise Batch17AuditError("Batch-17 evidence job set differs from selected keys")
    ...
    for ordinal, identity in enumerate(selected):
        if not isinstance(identity, Mapping):
            raise Batch17AuditError("Batch-17 selected identity is malformed")
        ...
        if len(job_inventory) != 55:
            raise Batch17AuditError("Batch-17 per-job evidence inventory count differs")
        try:
            ...
            raise Batch17AuditError(str(error)) from error
        if (
            tuple(physical["offline_dataset_exclusion_reasons"])
            != EXPECTED_DATASET_EXCLUSIONS[ordinal]
        ):
            raise Batch17AuditError("Batch-17 offline dataset exclusion reasons differ")
        raw_probe = json_object(
            read_regular_file_once(root / relative["raw_probe"]), label="Batch-17 raw probe"
        )
        current_receipt_ids = {
        ...
        if all_receipt_ids & current_receipt_ids:
            raise Batch17AuditError("Batch-17 physical receipt ID is reused")
        all_receipt_ids.update(current_receipt_ids)
        attempts.append(
            {
                "attempt_id": f"batch17-{ordinal + 1:02d}",
                "identity": identity,
    ...
    if (root / "packaged").exists() and any((root / "packaged").rglob("*")):
        raise Batch17AuditError("Batch-17 evidence contains packaged output")

    audit_path = Path(__file__).resolve()
    helper_path = project_root / "scripts/m2c/audit_s4_v4_batch10_collection.py"
    return {
        "schema_version": "M2CS4V4Batch17CollectionAuditV1",
        "status": "BLOCKED_ZERO_ELIGIBLE_V4_TRAIN_SAMPLES_BATCH17",
        ...
        "audit_implementation": {
            "path": "scripts/m2c/audit_s4_v4_batch17_collection.py",
...
def render_markdown(report: Mapping[str, Any]) -> str:
    return f"""# M2C S4 V4 Batch-17 collection audit

Status: `{report["status"]}`

The three outcome-blind preregistered TRAIN keys were consumed exactly once, with no retry or
replacement. All three completed strict eight-step V4 physical chains. Scene 19292 ended at the
unchanged REGRASP pregrasp-IK gate; scenes 19293 and 19305 ended at the unchanged REGRASP
contact gate. Host replay accepted all raw chains under the ADR-0025 32-detection schema and
produced `EMPTY` datasets.

## Observed outcomes

- scene 19292: steps 0-6 passed; REGRASP ended `PREGRASP_IK_GATE_REJECTED`.
- scene 19293: steps 0-6 passed; REGRASP ended `CONTACT_GATE_REJECTED`.
- scene 19305: steps 0-6 passed; REGRASP ended `CONTACT_GATE_REJECTED`; offline replay also
...
```bash
PYTHONPATH=src:scripts uv run python scripts/m2c/audit_s4_v4_batch17_collection.py \\
  --evidence-root /Users/gl/tzb-m2c-evidence/m2c-s4-v4-batch17-complete \\
  --expected-json reports/m2c-s4-v4-batch17-collection.json
```
"""
...
def main() -> int:
    ...
    if args.expected_json is not None and read_regular_file_once(args.expected_json) != encoded:
        raise SystemExit("Batch-17 audit differs from expected JSON")
    if args.markdown:
