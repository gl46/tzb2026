#!/usr/bin/env python3
"""Run exactly one consumed six-object ADR-0026 V2 diagnostic attempt."""
...
def run_one_v2(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if len(selected) != 1:
        raise TerminalDiagnosticError("V2 run ID is not uniquely preregistered")
    run = selected[0]
    if args.image != ISAAC_IMAGE:
        raise TerminalDiagnosticError("V2 image tag differs from preregistration")
    ...
    if (
        ...
        raise TerminalDiagnosticError("V2 source root must be root-owned 0555 evidence")
    for path, expected, label in (
        ...
        if path.parent.resolve(strict=True) != args.source_root.resolve(strict=True):
            raise TerminalDiagnosticError(f"V2 {label} is not under source root")
        if _sha256_file(path) != expected:
            raise TerminalDiagnosticError(f"V2 {label} digest differs from prereg")
    if _sha256_file(args.upstream_v4_probe) != resolved.prereg.upstream_v4_probe_sha256:
        raise TerminalDiagnosticError("V2 upstream probe digest differs from freeze")
    derived_bytes = derive_terminal_regrasp_diagnostic_probe_bytes_v2(
    ...
    if image_id != ISAAC_IMAGE_ID:
        raise TerminalDiagnosticError("V2 image ID differs from preregistration")
    ...
    if (
        ...
        raise TerminalDiagnosticError("V2 output root must be root-owned 0700")
    job_root = args.output_root / (
        f"run-{run.ordinal:02d}-{run.run_id.removeprefix('m2c-s4-terminal-diagnostic-')[:16]}"
    )
    if job_root.exists():
        raise FileExistsError(f"refusing to overwrite V2 diagnostic job: {job_root}")
    ...
    derived_probe = derived_root / "terminal-regrasp-diagnostic-probe-v2.py"
    ...
    claim = consume_diagnostic_run_v2(
        resolved=resolved,
        run_id=run.run_id,
    ...
    try:
        ...
        if stage.returncode != 0 or not _stage_is_valid(
            ...
            raise TerminalDiagnosticError("V2 stage failed its source-hash gate")
        ...
        if not raw_path.is_file():
            raise TerminalDiagnosticError("V2 probe emitted no terminal raw evidence")
        raw = parse_diagnostic_raw_bytes_v2(v4_auth.read_regular_file_once(raw_path))
        if raw.run_id != run.run_id:
            raise TerminalDiagnosticError("V2 raw evidence belongs to another run")
...
def main() -> int:
    ...
    parser.add_argument("--container-prefix", default="m2c-terminal-diagnostic-v2")
