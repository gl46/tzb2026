#!/usr/bin/env python3
"""Replay all three consumed Batch-18 attempts without upgrading evidence."""
...
PREREG_RELATIVE = Path("docs/decisions/M2C-S4-V4-TRAIN-COLLECTION-BATCH-18-PREREG.json")
PREREG_FILE_SHA256 = "2908c79c5dc9c6b396c46e9b59f7ba994df4aea70246a45c6f658697ce440fa7"
PREREG_SHA256 = "ea8201a202c6bd88a51c4ef6561f767b760477279d18650c15ff1833e0143518"
PREREG_COMMIT = "0eec6ffa731ec6b83d5130798425c67529ac1ecc"
SOURCE_SNAPSHOT_COMMIT = "4f37e3aa235190878438b79b8d925c63f5123427"
SOURCE_SNAPSHOT_INVENTORY_SHA256 = (
    "0f7b0432aa992321852de70bb3bbea2e5c1fd4ff7befddc160e7efe2e4d6ca44"
)
PREDECESSOR_RECEIPT_SHA256 = "7219de05761be061cb9b07c329ee27c22b746c9d3e4b48e89a2bb444a4df7d9f"
CLAIM_FILE_SHA256 = ("e6a2bf75d2125f32da1fa40d678870d6df904386017a9e07037a1298330d345e",)
EXPECTED_DETECTION_COUNTS = ((7, 7, 6, 8, 9, 12, 13, 12),)
...
class Batch18AuditError(Batch10AuditError):
    """Batch-18 bytes do not prove the bounded zero-eligible outcome."""
...
def _validate_claim_and_job(
    *,
    ...
    try:
        ...
        claim = json_object(claim_raw, label=f"Batch-18 claim {ordinal}")
        job = json_object(read_regular_file_once(root / relative["job"]), label="Batch-18 job")
    except FileNotFoundError as error:
        raise Batch18AuditError("Batch-18 claim or job evidence is missing") from error
    if sha256_bytes(claim_raw) != CLAIM_FILE_SHA256[ordinal] or claim_raw != projected_raw:
        raise Batch18AuditError("Batch-18 canonical and projected claim bytes differ")
    if not _claim_core_is_valid(claim):
        raise Batch18AuditError("Batch-18 claim semantic receipt hash is invalid")
    if (
        claim.get("schema_version") != "M2CS4V4CollectionConsumptionReceiptV1"
        or claim.get("batch_id") != "m2c-s4-v4-train-batch-18"
        or claim.get("ledger_namespace") != "M2C_S4_V4_COLLECTION_BATCH18"
        ...
        raise Batch18AuditError("Batch-18 claim identity or hash chain differs")
    ...
    if (
        ...
        raise Batch18AuditError("Batch-18 job does not bind the consumed claim")
    if inventory.get(relative["derived_probe"]) != claim.get("derived_probe_sha256"):
        raise Batch18AuditError("Batch-18 derived probe hash differs from its claim")
...
def build_report(*, evidence_root: Path, project_root: Path = ROOT) -> dict[str, Any]:
    ...
    if sha256_bytes(prereg_raw) != PREREG_FILE_SHA256:
        raise Batch18AuditError("Batch-18 preregistration bytes changed")
    if (
        _git_blob(project_root=project_root, commit=PREREG_COMMIT, relative_path=PREREG_RELATIVE)
        != prereg_raw
    ):
        raise Batch18AuditError("Batch-18 preregistration differs from its introduction blob")
    prereg = json_object(prereg_raw, label="Batch-18 preregistration")
    ...
    if (
        ...
        or prereg.get("stop_after_selected_keys") != 1
        ...
        raise Batch18AuditError("Batch-18 preregistration contract differs")
    inventory = _inventory(root)
    if len(inventory) != 56:
        raise Batch18AuditError("Batch-18 evidence inventory count differs")
    ...
    if job_keys != {str(item["matched_key"]) for item in selected}:
        raise Batch18AuditError("Batch-18 evidence job set differs from selected keys")
    ...
    for ordinal, identity in enumerate(selected):
        if not isinstance(identity, Mapping):
            raise Batch18AuditError("Batch-18 selected identity is malformed")
        ...
        if len(job_inventory) != 55:
            raise Batch18AuditError("Batch-18 per-job evidence inventory count differs")
        try:
            ...
            raise Batch18AuditError(str(error)) from error
        if (
            tuple(physical["offline_dataset_exclusion_reasons"])
            != EXPECTED_DATASET_EXCLUSIONS[ordinal]
        ):
            raise Batch18AuditError("Batch-18 offline dataset exclusion reasons differ")
        raw_probe = json_object(
            read_regular_file_once(root / relative["raw_probe"]), label="Batch-18 raw probe"
        )
        current_receipt_ids = {
        ...
        if all_receipt_ids & current_receipt_ids:
            raise Batch18AuditError("Batch-18 physical receipt ID is reused")
        all_receipt_ids.update(current_receipt_ids)
        attempts.append(
            {
                "attempt_id": f"batch18-{ordinal + 1:02d}",
                "identity": identity,
    ...
    if (root / "packaged").exists() and any((root / "packaged").rglob("*")):
        raise Batch18AuditError("Batch-18 evidence contains packaged output")
    ...
    return {
        ...
        "audit_implementation": {
            "path": "scripts/m2c/audit_s4_v4_batch18_collection.py",
...
def render_markdown(report: Mapping[str, Any]) -> str:
    return f"""# M2C S4 V4 Batch-18 collection audit
...
def main() -> int:
    ...
    if args.expected_json is not None and read_regular_file_once(args.expected_json) != encoded:
        raise SystemExit("Batch-18 audit differs from expected JSON")
    if args.markdown:
