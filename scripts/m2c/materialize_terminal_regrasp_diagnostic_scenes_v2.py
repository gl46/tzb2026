#!/usr/bin/env python3
"""Materialize the exact six-object ADR-0026 V2 diagnostic sources."""
...
def materialize_sources_v2(
    *, project_root: Path, prereg_path: Path, output_root: Path
    ...
    if output_root.exists():
        raise FileExistsError(f"refusing to overwrite V2 source root: {output_root}")
    ...
    if sha256_bytes(urdf_bytes) != CONTROLLED_URDF_SHA256:
        raise TerminalDiagnosticError("V2 controlled URDF digest mismatch")
    ...
    for run in resolved.prereg.runs:
        if run.source_base_seed not in cache:
            ...
            if full is None:
                raise TerminalDiagnosticError("V2 base seed is not a six-object scene")
        ...
        if (
            ...
            raise TerminalDiagnosticError("V2 source differs from preregistration")
    ...
    receipt = {
        "schema_version": "M2CTerminalDiagnosticMaterializationReceiptV2",
...
def main() -> int:
    ...
    receipt = materialize_sources_v2(
