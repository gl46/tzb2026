#!/usr/bin/env python3
"""Replay all three consumed Batch-12 attempts without upgrading evidence."""
...
PREREG_RELATIVE = Path("docs/decisions/M2C-S4-V4-TRAIN-COLLECTION-BATCH-12-PREREG.json")
PREREG_FILE_SHA256 = "60efb234013ecfcc2370a232b62b70e245656856515db0c9aa921d91db17a261"
PREREG_SHA256 = "13b11441dcfd5789baf0d5a6216ab87dfea841f9a710585ff5b2ce9d3180b262"
PREREG_COMMIT = "2f21d22bcbd40900f377d33eccea7a006fae3ad3"
SOURCE_SNAPSHOT_COMMIT = "d69b2a2ccaa1594fc38b4a5193f4fb6f640da1b2"
SOURCE_SNAPSHOT_INVENTORY_SHA256 = (
    "9155b7e329cf4b8dc27870f3d7f8bb572f16b8f42e08c5bbb195b5a95633a88e"
)
PREDECESSOR_RECEIPT_SHA256 = "c12978009de2982e717d4dccc22921784df7aa2e6b80ee4ff78ffa760994419a"
CLAIM_FILE_SHA256 = (
    "f3ed8e5c6d22b861b243dc7138c74d427bff46937499befc8a850eab09accfb3",
    "291a03a697a8a2a13cdd0df3b658dd0cca92735a37a9a5bb312d3bc61d7d67b6",
    "640dafa56c374f4ea532f912462ba55e2dba6285c8625c975f72e62e04f987d3",
)
...
class Batch12AuditError(Batch10AuditError):
    """Batch-12 bytes do not prove the bounded zero-eligible outcome."""
...
def _validate_claim_and_job(
    *,
    ...
    try:
        ...
        claim = json_object(claim_raw, label=f"Batch-12 claim {ordinal}")
        job = json_object(read_regular_file_once(root / relative["job"]), label="Batch-12 job")
    except FileNotFoundError as error:
        raise Batch12AuditError("Batch-12 claim or job evidence is missing") from error
    if sha256_bytes(claim_raw) != CLAIM_FILE_SHA256[ordinal] or claim_raw != projected_raw:
        raise Batch12AuditError("Batch-12 canonical and projected claim bytes differ")
    if not _claim_core_is_valid(claim):
        raise Batch12AuditError("Batch-12 claim semantic receipt hash is invalid")
    if (
        claim.get("schema_version") != "M2CS4V4CollectionConsumptionReceiptV1"
        or claim.get("batch_id") != "m2c-s4-v4-train-batch-12"
        or claim.get("ledger_namespace") != "M2C_S4_V4_COLLECTION_BATCH12"
        ...
        raise Batch12AuditError("Batch-12 claim identity or hash chain differs")
    ...
    if (
        ...
        raise Batch12AuditError("Batch-12 job does not bind the consumed claim")
    if inventory.get(relative["derived_probe"]) != claim.get("derived_probe_sha256"):
        raise Batch12AuditError("Batch-12 derived probe hash differs from its claim")
...
def build_report(*, evidence_root: Path, project_root: Path = ROOT) -> dict[str, Any]:
    ...
    if sha256_bytes(prereg_raw) != PREREG_FILE_SHA256:
        raise Batch12AuditError("Batch-12 preregistration bytes changed")
    if (
        _git_blob(project_root=project_root, commit=PREREG_COMMIT, relative_path=PREREG_RELATIVE)
        != prereg_raw
    ):
        raise Batch12AuditError("Batch-12 preregistration differs from its introduction blob")
    prereg = json_object(prereg_raw, label="Batch-12 preregistration")
    ...
    if (
        ...
        raise Batch12AuditError("Batch-12 preregistration contract differs")
    inventory = _inventory(root)
    if len(inventory) != 168:
        raise Batch12AuditError("Batch-12 evidence inventory count differs")
    ...
    if job_keys != {str(item["matched_key"]) for item in selected}:
        raise Batch12AuditError("Batch-12 evidence job set differs from selected keys")
    ...
    for ordinal, identity in enumerate(selected):
        if not isinstance(identity, Mapping):
            raise Batch12AuditError("Batch-12 selected identity is malformed")
        ...
        if len(job_inventory) != 55:
            raise Batch12AuditError("Batch-12 per-job evidence inventory count differs")
        try:
            ...
            raise Batch12AuditError(str(error)) from error
        if (
            tuple(physical["offline_dataset_exclusion_reasons"])
            != EXPECTED_DATASET_EXCLUSIONS[ordinal]
        ):
            raise Batch12AuditError("Batch-12 offline dataset exclusion reasons differ")
        raw_probe = json_object(
            read_regular_file_once(root / relative["raw_probe"]), label="Batch-12 raw probe"
        )
        current_receipt_ids = {
        ...
        if all_receipt_ids & current_receipt_ids:
            raise Batch12AuditError("Batch-12 physical receipt ID is reused")
        all_receipt_ids.update(current_receipt_ids)
        attempts.append(
            {
                "attempt_id": f"batch12-{ordinal + 1:02d}",
                "identity": identity,
    ...
    if (root / "packaged").exists() and any((root / "packaged").rglob("*")):
        raise Batch12AuditError("Batch-12 evidence contains packaged output")
    ...
    return {
        ...
        "audit_implementation": {
            "path": "scripts/m2c/audit_s4_v4_batch12_collection.py",
...
def render_markdown(report: Mapping[str, Any]) -> str:
    return f"""# M2C S4 V4 Batch-12 collection audit

Status: `{report["status"]}`

The three outcome-blind preregistered TRAIN keys were consumed exactly once, with no retry or
replacement. All three completed strict eight-step V4 physical chains. Scene 19189 ended at the
unchanged REGRASP pregrasp-IK gate; scenes 19190 and 19191 ended at the unchanged REGRASP
contact gate. Host replay accepted all raw chains under the ADR-0025 32-detection schema and
produced `EMPTY` datasets.

## Observed outcomes

- scene 19189: steps 0-6 passed; REGRASP ended `PREGRASP_IK_GATE_REJECTED`.
- scene 19190: steps 0-6 passed; REGRASP ended `CONTACT_GATE_REJECTED`.
- scene 19191: steps 0-6 passed; REGRASP ended `CONTACT_GATE_REJECTED`; offline replay also
...
```bash
PYTHONPATH=src:scripts uv run python scripts/m2c/audit_s4_v4_batch12_collection.py \\
  --evidence-root /Users/gl/tzb-m2c-evidence/m2c-s4-v4-batch12-complete \\
  --expected-json reports/m2c-s4-v4-batch12-collection.json
```
"""
...
def main() -> int:
    ...
    if args.expected_json is not None and read_regular_file_once(args.expected_json) != encoded:
        raise SystemExit("Batch-12 audit differs from expected JSON")
    if args.markdown:
