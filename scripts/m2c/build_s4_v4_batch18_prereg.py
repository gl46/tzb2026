#!/usr/bin/env python3
"""Build the outcome-blind, prereg-only Batch-18 V4 collection contract."""
...
PREREG_PATH = Path("docs/decisions/M2C-S4-V4-TRAIN-COLLECTION-BATCH-18-PREREG.json")
BATCH_ID = "m2c-s4-v4-train-batch-18"
LEDGER_NAMESPACE = "M2C_S4_V4_COLLECTION_BATCH18"
STOP_AFTER = 1
YIELD_REPORT_PATH = Path("reports/m2c-s4-training-eligibility-yield-adr0025.json")
YIELD_REPORT_SHA256 = "5dfa3a1649b6c1396723695437bd612fde7ad3fee55f8e8b94ff733d4c5eb6d7"
...
BATCH18_PRIOR_ATTEMPT_KEY_COUNT = 46


class Batch18PreregBuildError(ValueError):
    """The frozen inputs cannot produce the Batch-18 preregistration."""
...
def _json_object(payload: bytes, *, label: str) -> dict[str, Any]:
    try:
        ...
        raise Batch18PreregBuildError(f"{label} is not JSON") from error
    if not isinstance(value, dict):
        raise Batch18PreregBuildError(f"{label} is not an object")
    return value
...
def select_batch18_keys(*, project_root: Path, source_commit: str = "HEAD") -> list[dict[str, Any]]:
    ...
    for path in BATCH18_PRIOR_ATTEMPT_SOURCE_PATHS:
        ...
        if _sha256(raw) != expected_sha256:
            raise Batch18PreregBuildError(f"prior report SHA-256 differs: {path}")
        report = _json_object(raw, label=path)
        if report.get("schema_version") != expected_schema:
            raise Batch18PreregBuildError(f"prior report schema differs: {path}")
        attempts = report.get("attempts")
        if not isinstance(attempts, list):
            raise Batch18PreregBuildError(f"prior report lacks attempts: {path}")
        source_keys = {
        ...
        if len(source_keys) != expected_unique:
            raise Batch18PreregBuildError(f"prior report unique-key count differs: {path}")
        if prior_keys & source_keys:
            raise Batch18PreregBuildError("authoritative prior report identity sets overlap")
        prior_keys |= source_keys
    if len(prior_keys) != BATCH18_PRIOR_ATTEMPT_KEY_COUNT:
        raise Batch18PreregBuildError("authoritative prior identity union differs")
    ...
    if len(selected) != STOP_AFTER or len(selected_sdfs) != STOP_AFTER:
        raise Batch18PreregBuildError("manifest cannot supply the final new identity-disjoint key")
    return selected
...
def build_prereg(*, project_root: Path, source_commit: str = "HEAD") -> dict[str, Any]:
    ...
    if _sha256(_git_blob(root, commit, YIELD_REPORT_PATH.as_posix())) != YIELD_REPORT_SHA256:
        raise Batch18PreregBuildError("updated ADR-0025 yield report is not frozen in parent")
    if _sha256(_git_blob(root, commit, ADR0025_PATH.as_posix())) != ADR0025_SHA256:
        raise Batch18PreregBuildError("accepted ADR-0025 is not frozen in parent")
    ...
    prior_bindings = [
        ...
        for path in sorted(BATCH18_PRIOR_ATTEMPT_SOURCE_PATHS)
    ]
    core: dict[str, Any] = {
        ...
        "selected_keys": select_batch18_keys(project_root=root, source_commit=commit),
