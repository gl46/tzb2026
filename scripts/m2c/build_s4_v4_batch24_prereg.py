#!/usr/bin/env python3
"""Build the outcome-blind, prereg-only Batch-24 V4 collection contract."""
...
PREREG_PATH = Path("docs/decisions/M2C-S4-V4-TRAIN-COLLECTION-BATCH-24-PREREG.json")
BATCH_ID = "m2c-s4-v4-train-batch-24"
LEDGER_NAMESPACE = "M2C_S4_V4_COLLECTION_BATCH24"
STOP_AFTER = 3
...
YIELD_REPORT_SHA256 = "d5d4a524f391b1bdd24706fb02f92f867862d229b764400453124725e55cc78f"
...
BATCH24_PRIOR_ATTEMPT_SOURCE_PATHS = tuple(
    sorted(authorization.BATCH24_AUTHORITATIVE_PRIOR_ATTEMPT_SOURCES)
)
BATCH24_PRIOR_ATTEMPT_KEY_COUNT = authorization.BATCH24_AUTHORITATIVE_PRIOR_ATTEMPT_KEY_COUNT


class Batch24PreregBuildError(ValueError):
    """The frozen inputs cannot produce the Batch-24 preregistration."""
...
def _json_object(payload: bytes, *, label: str) -> dict[str, Any]:
    try:
        ...
        raise Batch24PreregBuildError(f"{label} is not JSON") from error
    if not isinstance(value, dict):
        raise Batch24PreregBuildError(f"{label} is not an object")
    return value
...
def select_batch24_keys(*, project_root: Path, source_commit: str = "HEAD") -> list[dict[str, Any]]:
    ...
    for path in BATCH24_PRIOR_ATTEMPT_SOURCE_PATHS:
        expected_sha256, expected_schema, expected_unique = (
            authorization.BATCH24_AUTHORITATIVE_PRIOR_ATTEMPT_SOURCES[path]
        )
        raw = _git_blob(project_root, source_commit, path)
        if _sha256(raw) != expected_sha256:
            raise Batch24PreregBuildError(f"prior report SHA-256 differs: {path}")
        report = _json_object(raw, label=path)
        if report.get("schema_version") != expected_schema:
            raise Batch24PreregBuildError(f"prior report schema differs: {path}")
        attempts = report.get("attempts")
        if not isinstance(attempts, list):
            raise Batch24PreregBuildError(f"prior report lacks attempts: {path}")
        source_keys = {
        ...
        if len(source_keys) != expected_unique:
            raise Batch24PreregBuildError(f"prior report unique-key count differs: {path}")
        if prior_keys & source_keys:
            raise Batch24PreregBuildError("authoritative prior report identity sets overlap")
        prior_keys |= source_keys
    if len(prior_keys) != BATCH24_PRIOR_ATTEMPT_KEY_COUNT:
        raise Batch24PreregBuildError("authoritative prior identity union differs")
    ...
    if len(selected) != STOP_AFTER or len(selected_sdfs) != STOP_AFTER:
        raise Batch24PreregBuildError("extension cannot supply three new SDF-balanced identities")
    return selected
...
def build_prereg(*, project_root: Path, source_commit: str = "HEAD") -> dict[str, Any]:
    ...
    if _sha256(_git_blob(root, commit, YIELD_REPORT_PATH.as_posix())) != YIELD_REPORT_SHA256:
        raise Batch24PreregBuildError("ADR-0025 yield report is not frozen in parent")
    if _sha256(_git_blob(root, commit, ADR0025_PATH.as_posix())) != ADR0025_SHA256:
        raise Batch24PreregBuildError("accepted ADR-0025 is not frozen in parent")
    ...
    prior_bindings = [
        {
            "path": path,
            "sha256": authorization.BATCH24_AUTHORITATIVE_PRIOR_ATTEMPT_SOURCES[path][0],
        }
        for path in BATCH24_PRIOR_ATTEMPT_SOURCE_PATHS
    ]
    core: dict[str, Any] = {
        ...
        "selected_keys": select_batch24_keys(project_root=root, source_commit=commit),
...
def write_create_only(path: Path, payload: bytes) -> None:
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise OSError("failed to publish Batch-24 preregistration")
            view = view[written:]
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
