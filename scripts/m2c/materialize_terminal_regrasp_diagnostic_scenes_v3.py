#!/usr/bin/env python3
"""Materialize the exact six-object ADR-0026 V3 diagnostic sources."""
...
def materialize_sources_v3(
    *, project_root: Path, prereg_path: Path, output_root: Path
) -> dict[str, object]:
    ...
    if output_root.exists():
        raise FileExistsError(f"refusing to overwrite V3 source root: {output_root}")
    ...
    if sha256_bytes(urdf_bytes) != CONTROLLED_URDF_SHA256:
        raise TerminalDiagnosticError("V3 controlled URDF digest mismatch")
    ...
    for run in resolved.prereg.runs:
        if run.source_base_seed not in cache:
            ...
            if full is None:
                raise TerminalDiagnosticError("V3 base seed is not a six-object scene")
        ...
        if (
            ...
            raise TerminalDiagnosticError("V3 source differs from preregistration")
    ...
    receipt = {
        "schema_version": "M2CTerminalDiagnosticMaterializationReceiptV3",
        "status": "MATERIALIZED_SIX_OBJECT_DIAGNOSTIC_ONLY_NO_ISAAC_EXECUTION",
...
def main() -> int:
    ...
    receipt = materialize_sources_v3(
