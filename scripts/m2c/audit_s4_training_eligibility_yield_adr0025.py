...
def build_report(*, project_root: Path) -> dict[str, Any]:
    ...
    for spec, report in parsed[:2]:
        for attempt in report["attempts"]:
            if str(attempt["classification"]).startswith("RAW_V3_"):
                ...
                complete_chain_count += 1
    for spec, batch_report in parsed[-9:]:
    ...
    complete_chain_count += 1
    if complete_chain_count != 37 or taxonomy != Counter(
        {
    ...
    return {
        ...
        "root_cause_finding": {
            ...
            "complete_v4_chains_with_steps_0_through_6_all_gates_pass": 26,
            "complete_chains_ending_in_contact_or_controller_rejection": 25,
            "complete_chains_ending_in_pregrasp_ik_rejection": 11,
        ...
        "blockers": [
            "ZERO_ELIGIBLE_TRAINING_EPISODES_ACROSS_46_UNIQUE_TRAIN_IDENTITIES",
...
This report replays fifteen immutable collection reports plus the governed offline replay of scene 19083. It does not collect, execute physics, train, run a model rollout, or perform formal Q-B evaluation.

## Measured yield

- Unique TRAIN identities: **{report["identity_audit"]["unique_train_identities_total"]}** (V3: 11; V4: 35)
- Complete eight-step physical chains: **{yield_data["complete_eight_step_chains"]}**
- Eligible and packaged training episodes: **0**
- Eligible yield per attempted identity: **0/46 = 0.0**
- Eligible yield conditional on a complete chain: **0/37 = 0.0**
