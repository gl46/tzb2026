#!/usr/bin/env python3
"""Freeze 36 outcome-blind, TRAIN-only ADR-0021 V3 collection keys.
...
overlap with every frozen V2 TRAIN/SMOKE, V4 Q-A, or S6 evaluation key.
"""
...
OUTPUT = PROJECT / "configs/m2c_s4_v3_training_keys.json"
ADR = PROJECT / "docs/decisions/ADR-0021-m2c-public-semantic-candidate-contract.md"
...
PUBLIC_TRACKS_V3 = PROJECT / "src/xh_agent/policy/qrm_lite/public_tracks_v3.py"
EXPECTED_SOURCE_HASHES = {
    "docs/decisions/ADR-0021-m2c-public-semantic-candidate-contract.md": (
...
SCHEMA_VERSION = "M2CS4V3TrainingKeyManifestV1"
TRAIN_KEY_COUNT = 36
TRAIN_SEED_RANGE = range(16_000, 19_000)
...
def _key_record(seed: int, ordinal: int, template: str) -> dict[str, Any] | None:
    if split(seed) != "train":
        raise ValueError(f"V3 TRAIN seed {seed} is not generator split train")
    ...
    if receipt["split"] != "train" or receipt["part_count"] != 6:
        raise ValueError(f"V3 TRAIN seed {seed} failed offline geometry contract")
    identity = {
        "scene_seed": seed,
        ...
        "offline_geometry_admission": {
            ...
            "target_clearance_after_relocation_m": receipt[
                "target_clearance_after_relocation_m"
            ],
            "target_clearance_after_relocation_ok": receipt[
                "target_clearance_after_relocation_ok"
            ],
        },
    ...
    return {**identity, "matched_key": "m2c-s4-v3-train-" + canonical_sha256(identity)}


def build_manifest() -> dict[str, Any]:
    actual_bindings = {
        path: sha256_file(PROJECT / path) for path in EXPECTED_SOURCE_HASHES
    }
    mismatches = {
    ...
    if mismatches:
        raise ValueError(f"frozen V3 manifest source hash mismatch: {mismatches}")
    category_audit = _json(CATEGORY_AUDIT)
    if (
        category_audit.get("status")
        != "PASS_CATEGORY_CARRIES_DECLARED_ATTRIBUTE_VOCABULARY"
        ...
        raise ValueError("ADR-0021 category-vocabulary prerequisite is not passing")
    ...
    for seed in TRAIN_SEED_RANGE:
        ...
        if (
            ...
            raise AssertionError("V3 TRAIN key overlaps a frozen excluded identity")
        records.append(record)
        if len(records) == TRAIN_KEY_COUNT:
            break
    if len(records) != TRAIN_KEY_COUNT:
        raise ValueError("V3 seed range produced fewer than 36 admissible TRAIN keys")
    ...
    if (
        ...
        raise AssertionError("V3 TRAIN identities are not internally unique")

    payload: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "status": "FROZEN_TRAIN_ONLY_BEFORE_ANY_V3_COLLECTION",
        ...
        "any_v3_collection_observed_before_freeze": False,
        "selection_implementation": {
            "path": str(Path(__file__).resolve().relative_to(PROJECT)),
            "sha256": sha256_file(Path(__file__).resolve()),
        },
        "candidate_implementation": {
            "path": str(PUBLIC_TRACKS_V3.relative_to(PROJECT)),
            "sha256": sha256_file(PUBLIC_TRACKS_V3),
        },
        "selection_protocol": {
...
def write_create_only(path: Path, payload: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise OSError("failed to publish V3 training manifest")
            view = view[written:]
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
