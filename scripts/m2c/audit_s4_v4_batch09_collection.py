#!/usr/bin/env python3
"""Replay all three consumed Batch-09 attempts without upgrading evidence."""
...
PREREG_RELATIVE = Path("docs/decisions/M2C-S4-V4-TRAIN-COLLECTION-BATCH-09-PREREG.json")
PREREG_FILE_SHA256 = "e49496c9898f96230c6521ea23cc8aeec9c78326a85f5b745966bba95fc66238"
PREREG_SHA256 = "98a5f609d33c0cea81fcc991945c5e629f8a1416d3a438efba619c8c53cd1e8b"
PREREG_COMMIT = "5d4e8845b45358c656997b9b9ebb8b0dab30e9c2"
SOURCE_SNAPSHOT_COMMIT = "8fd4edc42a35460d9ba3bc0f489982254dfdb5b3"
SOURCE_SNAPSHOT_INVENTORY_SHA256 = (
    "3f5a035b6be729aee80b31a5a720bc847e5b813bafa4b97448bdd455114c33c5"
)
PREDECESSOR_RECEIPT_SHA256 = "9c072d53196140a20e944094ac5874ad68921ece1ab6cf3a973f3c37744bd11d"
...
CLAIM_FILE_SHA256 = (
    "e4ee0fdf4e70c01caa85a2bd1701e11bc796e382a3cf3b2f0326c5fdbaab6a48",
    "0a8c1aad6b63ff7a615cfc583ad2f629b9e4beee125cff4d583e3441cca3c6f4",
    "9cd7b03a5f88a898f25de38132ea5349029a56e9c6d2218d135fd5ac80ef8343",
)
...
class Batch09AuditError(Batch04AuditError):
    """Batch-09 bytes do not prove the bounded zero-eligible outcome."""
...
def _git_blob(*, project_root: Path, commit: str, relative_path: Path) -> bytes:
    ...
    if completed.returncode != 0:
        raise Batch09AuditError("Batch-09 preregistration Git blob is unavailable")
    ...
    if isinstance(value, Mapping):
        for key, nested in value.items():
            if (
                ...
                raise Batch09AuditError(f"forbidden boundary is not false: {path}.{key}")
...
def _dataset_asset(probe_root: Path, uri: str) -> Path:
    ...
    if not uri.startswith(prefix):
        raise Batch09AuditError("raw capture asset is not a dataset URI")
    relative = PurePosixPath(uri.removeprefix(prefix))
    if relative.is_absolute() or ".." in relative.parts:
        raise Batch09AuditError("raw capture asset escaped the probe root")
...
def _validate_claim_and_job(
    *,
    ...
    try:
        ...
        claim = json_object(claim_raw, label=f"Batch-09 claim {ordinal}")
        job = json_object(read_regular_file_once(root / relative["job"]), label="Batch-09 job")
    except FileNotFoundError as error:
        raise Batch09AuditError("Batch-09 claim or job evidence is missing") from error
    if sha256_bytes(claim_raw) != CLAIM_FILE_SHA256[ordinal] or claim_raw != projected_raw:
        raise Batch09AuditError("Batch-09 canonical and projected claim bytes differ")
    if not _claim_core_is_valid(claim):
        raise Batch09AuditError("Batch-09 claim semantic receipt hash is invalid")
    if (
        claim.get("schema_version") != "M2CS4V4CollectionConsumptionReceiptV1"
        or claim.get("batch_id") != "m2c-s4-v4-train-batch-09"
        or claim.get("ledger_namespace") != "M2C_S4_V4_COLLECTION_BATCH09"
        ...
        raise Batch09AuditError("Batch-09 claim identity or hash chain differs")
    ...
    if (
        ...
        raise Batch09AuditError("Batch-09 job does not bind the consumed claim")
    if inventory[relative["derived_probe"]] != claim.get("derived_probe_sha256"):
        raise Batch09AuditError("Batch-09 derived probe hash differs from its claim")
...
def _validate_physical_attempt(
    *,
    ...
    raw_probe = json_object(raw_probe_bytes, label="Batch-09 raw physical probe")
    ...
    if (
        ...
        raise Batch09AuditError("Batch-09 Isaac lifecycle or stage acceptance differs")
    ...
    if (
        ...
        raise Batch09AuditError("Batch-09 clean stage hash differs")
    ...
    if not isinstance(authorization, Mapping) or not isinstance(chain_raw, Mapping):
        raise Batch09AuditError("Batch-09 raw probe lacks authorization or physical chain")
    if not isinstance(captures, list) or len(captures) != 8:
        raise Batch09AuditError("Batch-09 raw probe does not contain eight captures")
    if (
        ...
        raise Batch09AuditError("Batch-09 raw chain is not bound to the consumed claim")
    try:
        chain = M2CPathBlockedRawProbeChainV4.model_validate(chain_raw)
    except ValidationError as error:
        raise Batch09AuditError("Batch-09 raw physical chain schema is invalid") from error
    if (
        ...
        raise Batch09AuditError("Batch-09 raw physical chain identity differs")
    ...
    for index, (raw_capture, expected_count, step) in enumerate(
        ...
        if not isinstance(raw_capture, Mapping):
            raise Batch09AuditError("Batch-09 raw public capture is not an object")
        try:
            capture = M2CV4RawPublicAssociationCaptureV2.model_validate(raw_capture)
        except ValidationError as error:
            raise Batch09AuditError("Batch-09 raw public capture schema is invalid") from error
        if len(capture.detections) != expected_count:
            raise Batch09AuditError("Batch-09 raw public detection count differs")
        if step.observation.capture_receipt_sha256 != canonical_sha256(raw_capture):
            raise Batch09AuditError("Batch-09 raw observation capture receipt differs")
        if (
            ...
            raise Batch09AuditError("Batch-09 raw observation does not bind its capture")
        for kind in ("rgb", "depth"):
            ...
            if sha256_bytes(read_regular_file_once(asset)) != raw_capture[f"{kind}_sha256"]:
                raise Batch09AuditError("Batch-09 raw RGB-D asset hash differs")
        if len(step.physical_receipts) != 1:
            raise Batch09AuditError("Batch-09 physical step does not have one receipt")
        raw_receipt = chain_raw["steps"][index]["physical_receipts"][0]
        receipt = step.physical_receipts[0]
        if not isinstance(raw_receipt, Mapping):
            raise Batch09AuditError("Batch-09 raw physical receipt is not an object")
        ...
        if (
            ...
            raise Batch09AuditError("Batch-09 physical receipt or safety gates differ")
        receipt_ids.append(receipt.receipt_id)
    if len(set(receipt_ids)) != 8:
        raise Batch09AuditError("Batch-09 physical receipt IDs are not unique")
    terminal = chain.steps[7].physical_receipts[0]
    if terminal.execution_measurements.get("status") != expected_final_status:
        raise Batch09AuditError("Batch-09 terminal execution status differs")
    if raw_probe.get("status") != "PASS" or raw_probe.get("not_policy_rollout") is not True:
        raise Batch09AuditError("Batch-09 raw probe status or rollout boundary differs")
...
def build_report(*, evidence_root: Path, project_root: Path = ROOT) -> dict[str, Any]:
    ...
    if sha256_bytes(prereg_raw) != PREREG_FILE_SHA256:
        raise Batch09AuditError("Batch-09 preregistration bytes changed")
    if (
        _git_blob(
        ...
        raise Batch09AuditError("Batch-09 preregistration differs from its introduction blob")
    prereg = json_object(prereg_raw, label="Batch-09 preregistration")
    ...
    if (
        ...
        raise Batch09AuditError("Batch-09 preregistration contract differs")
    inventory = _inventory(root)
    if len(inventory) != 168:
        raise Batch09AuditError("Batch-09 evidence inventory count differs")
    ...
    if job_keys != {str(item["matched_key"]) for item in selected}:
        raise Batch09AuditError("Batch-09 evidence job set differs from selected keys")
    ...
    for ordinal, identity in enumerate(selected):
        if not isinstance(identity, Mapping):
            raise Batch09AuditError("Batch-09 selected identity is malformed")
        ...
        if len(job_inventory) != 55:
            raise Batch09AuditError("Batch-09 per-job evidence inventory count differs")
        ...
        if (
            ...
            raise Batch09AuditError("Batch-09 offline dataset exclusion reasons differ")
        raw_probe = json_object(
            read_regular_file_once(root / relative["raw_probe"]), label="Batch-09 raw probe"
        )
        current_receipt_ids = {
        ...
        if all_receipt_ids & current_receipt_ids:
            raise Batch09AuditError("Batch-09 physical receipt ID is reused across attempts")
        all_receipt_ids.update(current_receipt_ids)
        attempts.append(
            {
                "attempt_id": f"batch09-{ordinal + 1:02d}",
                "identity": identity,
    ...
    return {
        ...
        "audit_implementation": {
            "path": "scripts/m2c/audit_s4_v4_batch09_collection.py",
...
def render_markdown(report: Mapping[str, Any]) -> str:
    return f"""# M2C S4 V4 Batch-09 collection audit
...
```bash
PYTHONPATH=src:scripts uv run python scripts/m2c/audit_s4_v4_batch09_collection.py \
  --evidence-root /Users/gl/tzb-m2c-evidence/m2c-s4-v4-batch09-complete \
  --expected-json reports/m2c-s4-v4-batch09-collection.json
```
"""
...
def main() -> int:
    ...
    if args.expected_json is not None and read_regular_file_once(args.expected_json) != encoded:
        raise SystemExit("Batch-09 audit differs from expected JSON")
    if args.markdown:
