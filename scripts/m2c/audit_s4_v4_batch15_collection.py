#!/usr/bin/env python3
"""Replay all three consumed Batch-15 attempts without upgrading evidence."""
...
PREREG_RELATIVE = Path("docs/decisions/M2C-S4-V4-TRAIN-COLLECTION-BATCH-15-PREREG.json")
PREREG_FILE_SHA256 = "12a4873bf83c86a9ad6609c1ea5c0735bc3e7ef6acd0ef2662b2647c63f06a07"
PREREG_SHA256 = "8c99ad37a2448a628ab8f4e2980268ab5878aac1772174147222511d83609c3b"
PREREG_COMMIT = "b8c87c03041df1893bb5bba4648670db8941a4f7"
SOURCE_SNAPSHOT_COMMIT = "a8190c73e7251d480885f84942a5ecb60016398d"
SOURCE_SNAPSHOT_INVENTORY_SHA256 = (
    "aec67d6eb7d9313f99bd74649c8e2f268a6865b028c8733078de9f278da75547"
)
PREDECESSOR_RECEIPT_SHA256 = "f047c50129df7ea9af42d02be03ca90dd5d3e0f0667e21b8095a8a13432bcbc1"
CLAIM_FILE_SHA256 = (
    "55341eb74eee6bb6069a9c1a7f6e0ad4ed3e88f16107b9e4545881a84f38d2c4",
    "21c0a4bbad2c1d306dfe07a682fc95700c6f96b3699e93d5c3e918c74adf24dd",
    "81ad037dda271096ca47a27bc29087b6fd080a73e965aabf08891c4e7fd962fa",
)
...
class Batch15AuditError(Batch10AuditError):
    """Batch-15 bytes do not prove the bounded zero-eligible outcome."""
...
def _validate_claim_and_job(
    *,
    ...
    try:
        ...
        claim = json_object(claim_raw, label=f"Batch-15 claim {ordinal}")
        job = json_object(read_regular_file_once(root / relative["job"]), label="Batch-15 job")
    except FileNotFoundError as error:
        raise Batch15AuditError("Batch-15 claim or job evidence is missing") from error
    if sha256_bytes(claim_raw) != CLAIM_FILE_SHA256[ordinal] or claim_raw != projected_raw:
        raise Batch15AuditError("Batch-15 canonical and projected claim bytes differ")
    if not _claim_core_is_valid(claim):
        raise Batch15AuditError("Batch-15 claim semantic receipt hash is invalid")
    if (
        claim.get("schema_version") != "M2CS4V4CollectionConsumptionReceiptV1"
        or claim.get("batch_id") != "m2c-s4-v4-train-batch-15"
        or claim.get("ledger_namespace") != "M2C_S4_V4_COLLECTION_BATCH15"
        ...
        raise Batch15AuditError("Batch-15 claim identity or hash chain differs")
    ...
    if (
        ...
        raise Batch15AuditError("Batch-15 job does not bind the consumed claim")
    if inventory.get(relative["derived_probe"]) != claim.get("derived_probe_sha256"):
        raise Batch15AuditError("Batch-15 derived probe hash differs from its claim")
...
def build_report(*, evidence_root: Path, project_root: Path = ROOT) -> dict[str, Any]:
    ...
    if sha256_bytes(prereg_raw) != PREREG_FILE_SHA256:
        raise Batch15AuditError("Batch-15 preregistration bytes changed")
    if (
        _git_blob(project_root=project_root, commit=PREREG_COMMIT, relative_path=PREREG_RELATIVE)
        != prereg_raw
    ):
        raise Batch15AuditError("Batch-15 preregistration differs from its introduction blob")
    prereg = json_object(prereg_raw, label="Batch-15 preregistration")
    ...
    if (
        ...
        raise Batch15AuditError("Batch-15 preregistration contract differs")
    inventory = _inventory(root)
    if len(inventory) != 168:
        raise Batch15AuditError("Batch-15 evidence inventory count differs")
    ...
    if job_keys != {str(item["matched_key"]) for item in selected}:
        raise Batch15AuditError("Batch-15 evidence job set differs from selected keys")
    ...
    for ordinal, identity in enumerate(selected):
        if not isinstance(identity, Mapping):
            raise Batch15AuditError("Batch-15 selected identity is malformed")
        ...
        if len(job_inventory) != 55:
            raise Batch15AuditError("Batch-15 per-job evidence inventory count differs")
        try:
            ...
            raise Batch15AuditError(str(error)) from error
        if (
            tuple(physical["offline_dataset_exclusion_reasons"])
            != EXPECTED_DATASET_EXCLUSIONS[ordinal]
        ):
            raise Batch15AuditError("Batch-15 offline dataset exclusion reasons differ")
        raw_probe = json_object(
            read_regular_file_once(root / relative["raw_probe"]), label="Batch-15 raw probe"
        )
        current_receipt_ids = {
        ...
        if all_receipt_ids & current_receipt_ids:
            raise Batch15AuditError("Batch-15 physical receipt ID is reused")
        all_receipt_ids.update(current_receipt_ids)
        attempts.append(
            {
                "attempt_id": f"batch15-{ordinal + 1:02d}",
                "identity": identity,
    ...
    if (root / "packaged").exists() and any((root / "packaged").rglob("*")):
        raise Batch15AuditError("Batch-15 evidence contains packaged output")
    ...
    return {
        ...
        "audit_implementation": {
            "path": "scripts/m2c/audit_s4_v4_batch15_collection.py",
...
def render_markdown(report: Mapping[str, Any]) -> str:
    return f"""# M2C S4 V4 Batch-15 collection audit
...
def main() -> int:
    ...
    if args.expected_json is not None and read_regular_file_once(args.expected_json) != encoded:
        raise SystemExit("Batch-15 audit differs from expected JSON")
    if args.markdown:
