#!/usr/bin/env python3
"""Replay all three consumed Batch-13 attempts without upgrading evidence."""
...
PREREG_RELATIVE = Path("docs/decisions/M2C-S4-V4-TRAIN-COLLECTION-BATCH-13-PREREG.json")
PREREG_FILE_SHA256 = "13ad1dd403df3ac66e804eed38ce1c0c2d9adeaea5e7b9a79fdb8e49787e576b"
PREREG_SHA256 = "690b3efbb20e3ad1b239342c929f485433d5832dd2614aedbaa6fa17031d8247"
PREREG_COMMIT = "c5139e9ac6e1924517da29933e357bbfce63a5e0"
SOURCE_SNAPSHOT_COMMIT = "1b8417cb08e5ed9ea4554c3f97dce414655c4996"
SOURCE_SNAPSHOT_INVENTORY_SHA256 = (
    "3095588ad07c02c8c37bbc0479f8ff4cd0945109764375e92421c0b9782058fe"
)
PREDECESSOR_RECEIPT_SHA256 = "410d7745fa0c829f015274806444d5f21b077879e22be8af8941ac0b4ab10b7d"
CLAIM_FILE_SHA256 = (
    "4783d802613f17b7166b87dc924c25f13094be05dc6b1a528c4677d895d74f0a",
    "7b82e10fb43dcbf20e1ba75f799646a619b23068654de25d6af9189228330472",
    "3375bc5f107262601e222894879eb4e824eba5ccc82ac8b025a8b5b576e1170f",
)
...
class Batch13AuditError(Batch10AuditError):
    """Batch-13 bytes do not prove the bounded zero-eligible outcome."""
...
def _validate_claim_and_job(
    *,
    ...
    try:
        ...
        claim = json_object(claim_raw, label=f"Batch-13 claim {ordinal}")
        job = json_object(read_regular_file_once(root / relative["job"]), label="Batch-13 job")
    except FileNotFoundError as error:
        raise Batch13AuditError("Batch-13 claim or job evidence is missing") from error
    if sha256_bytes(claim_raw) != CLAIM_FILE_SHA256[ordinal] or claim_raw != projected_raw:
        raise Batch13AuditError("Batch-13 canonical and projected claim bytes differ")
    if not _claim_core_is_valid(claim):
        raise Batch13AuditError("Batch-13 claim semantic receipt hash is invalid")
    if (
        claim.get("schema_version") != "M2CS4V4CollectionConsumptionReceiptV1"
        or claim.get("batch_id") != "m2c-s4-v4-train-batch-13"
        or claim.get("ledger_namespace") != "M2C_S4_V4_COLLECTION_BATCH13"
        ...
        raise Batch13AuditError("Batch-13 claim identity or hash chain differs")
    ...
    if (
        ...
        raise Batch13AuditError("Batch-13 job does not bind the consumed claim")
    if inventory.get(relative["derived_probe"]) != claim.get("derived_probe_sha256"):
        raise Batch13AuditError("Batch-13 derived probe hash differs from its claim")
...
def build_report(*, evidence_root: Path, project_root: Path = ROOT) -> dict[str, Any]:
    ...
    if sha256_bytes(prereg_raw) != PREREG_FILE_SHA256:
        raise Batch13AuditError("Batch-13 preregistration bytes changed")
    if (
        _git_blob(project_root=project_root, commit=PREREG_COMMIT, relative_path=PREREG_RELATIVE)
        != prereg_raw
    ):
        raise Batch13AuditError("Batch-13 preregistration differs from its introduction blob")
    prereg = json_object(prereg_raw, label="Batch-13 preregistration")
    ...
    if (
        ...
        raise Batch13AuditError("Batch-13 preregistration contract differs")
    inventory = _inventory(root)
    if len(inventory) != 168:
        raise Batch13AuditError("Batch-13 evidence inventory count differs")
    ...
    if job_keys != {str(item["matched_key"]) for item in selected}:
        raise Batch13AuditError("Batch-13 evidence job set differs from selected keys")
    ...
    for ordinal, identity in enumerate(selected):
        if not isinstance(identity, Mapping):
            raise Batch13AuditError("Batch-13 selected identity is malformed")
        ...
        if len(job_inventory) != 55:
            raise Batch13AuditError("Batch-13 per-job evidence inventory count differs")
        try:
            ...
            raise Batch13AuditError(str(error)) from error
        if (
            tuple(physical["offline_dataset_exclusion_reasons"])
            != EXPECTED_DATASET_EXCLUSIONS[ordinal]
        ):
            raise Batch13AuditError("Batch-13 offline dataset exclusion reasons differ")
        raw_probe = json_object(
            read_regular_file_once(root / relative["raw_probe"]), label="Batch-13 raw probe"
        )
        current_receipt_ids = {
        ...
        if all_receipt_ids & current_receipt_ids:
            raise Batch13AuditError("Batch-13 physical receipt ID is reused")
        all_receipt_ids.update(current_receipt_ids)
        attempts.append(
            {
                "attempt_id": f"batch13-{ordinal + 1:02d}",
                "identity": identity,
    ...
    if (root / "packaged").exists() and any((root / "packaged").rglob("*")):
        raise Batch13AuditError("Batch-13 evidence contains packaged output")
    ...
    return {
        ...
        "audit_implementation": {
            "path": "scripts/m2c/audit_s4_v4_batch13_collection.py",
...
def render_markdown(report: Mapping[str, Any]) -> str:
    return f"""# M2C S4 V4 Batch-13 collection audit
...
def main() -> int:
    ...
    if args.expected_json is not None and read_regular_file_once(args.expected_json) != encoded:
        raise SystemExit("Batch-13 audit differs from expected JSON")
    if args.markdown:
