#!/usr/bin/env python3
"""Replay the three consumed Batch-11 attempts without upgrading evidence."""
...
PREREG_RELATIVE = Path("docs/decisions/M2C-S4-V4-TRAIN-COLLECTION-BATCH-11-PREREG.json")
PREREG_FILE_SHA256 = "d207d25da2b4d36797ae718a9edf04a08ff895687319b299f71047762e073722"
PREREG_SHA256 = "3a8314aed01f463c88777585922c1d07e2c7832eb7923ea4abea7ec49e5ef3e7"
PREREG_COMMIT = "1580e321d9ac31bf0a535b33c7772d2753dec6d0"
SOURCE_SNAPSHOT_COMMIT = "ec5c6c2cfb94f5de51cda20ec79701df294c10b9"
SOURCE_SNAPSHOT_INVENTORY_SHA256 = (
    "dbb85310b544a74234be965f1a36fba73c7187aa54230e9377f30dff755b152b"
)
PREDECESSOR_RECEIPT_SHA256 = "251ca35d51e6199a0100b50a4ed6a7098a3ac95d4532b2b739fe78484e00cbda"
CLAIM_FILE_SHA256 = (
    "1e7a5c54b6cde1efef8422817d41f2132e33bf1a14a8020ef9221d19cf197725",
    "4facf0331fbc6c4224e382a7b1dbaf99ac6b533920cb319cc5b8f64606ed6cb8",
    "d124697c8d1dd492520771889be5050b014ac635c03548e7012498c159e96721",
)
...
class Batch11AuditError(Batch10AuditError):
    """Batch-11 bytes do not prove the bounded mixed zero-eligible outcome."""
...
def _validate_claim_and_job(
    *,
    ...
    try:
        ...
        claim = json_object(claim_raw, label=f"Batch-11 claim {ordinal}")
        job = json_object(read_regular_file_once(root / relative["job"]), label="Batch-11 job")
    except FileNotFoundError as error:
        raise Batch11AuditError("Batch-11 claim or job evidence is missing") from error
    if sha256_bytes(claim_raw) != CLAIM_FILE_SHA256[ordinal] or claim_raw != projected_raw:
        raise Batch11AuditError("Batch-11 canonical and projected claim bytes differ")
    if not _claim_core_is_valid(claim):
        raise Batch11AuditError("Batch-11 claim semantic receipt hash is invalid")
    if (
        claim.get("schema_version") != "M2CS4V4CollectionConsumptionReceiptV1"
        or claim.get("batch_id") != "m2c-s4-v4-train-batch-11"
        or claim.get("ledger_namespace") != "M2C_S4_V4_COLLECTION_BATCH11"
        ...
        raise Batch11AuditError("Batch-11 claim identity or hash chain differs")
    ...
    if (
        ...
        raise Batch11AuditError("Batch-11 job does not bind the consumed claim")
    if inventory.get(relative["derived_probe"]) != claim.get("derived_probe_sha256"):
        raise Batch11AuditError("Batch-11 derived probe hash differs from its claim")
...
def _validate_stage_failure(
    *,
    ...
    if (
        ...
        raise Batch11AuditError("Batch-11 stage exit-139 evidence differs")
    return {
...
def build_report(*, evidence_root: Path, project_root: Path = ROOT) -> dict[str, Any]:
    ...
    if sha256_bytes(prereg_raw) != PREREG_FILE_SHA256:
        raise Batch11AuditError("Batch-11 preregistration bytes changed")
    if (
        _git_blob(
        ...
        raise Batch11AuditError("Batch-11 preregistration differs from its introduction blob")
    prereg = json_object(prereg_raw, label="Batch-11 preregistration")
    ...
    if (
        ...
        raise Batch11AuditError("Batch-11 preregistration contract differs")
    inventory = _inventory(root)
    if len(inventory) != 118:
        raise Batch11AuditError("Batch-11 evidence inventory count differs")
    ...
    if job_keys != {str(item["matched_key"]) for item in selected}:
        raise Batch11AuditError("Batch-11 evidence job set differs from selected keys")
    ...
    for ordinal, identity in enumerate(selected):
        if not isinstance(identity, Mapping):
            raise Batch11AuditError("Batch-11 selected identity is malformed")
        ...
        if len(job_inventory) != expected_file_count:
            raise Batch11AuditError("Batch-11 per-job evidence inventory count differs")
        if ordinal == 0:
            ...
            if all_receipt_ids & current_receipt_ids:
                raise Batch11AuditError("Batch-11 physical receipt ID is reused")
            all_receipt_ids.update(current_receipt_ids)
            outcome.update(
                {
                    "classification": "RAW_V4_FINAL_FALSE_REGRASP_CONTACT_GATE_REJECTED",
        ...
        attempts.append(
            {
                "attempt_id": f"batch11-{ordinal + 1:02d}",
                "identity": identity,
    ...
    if (root / "packaged").exists() and any((root / "packaged").rglob("*")):
        raise Batch11AuditError("Batch-11 evidence contains packaged output")
    ...
    return {
        ...
        "audit_implementation": {
            "path": "scripts/m2c/audit_s4_v4_batch11_collection.py",
...
def render_markdown(report: Mapping[str, Any]) -> str:
    return f"""# M2C S4 V4 Batch-11 collection audit
...
def main() -> int:
    ...
    if args.expected_json is not None and read_regular_file_once(args.expected_json) != encoded:
        raise SystemExit("Batch-11 audit differs from expected JSON")
    if args.markdown:
