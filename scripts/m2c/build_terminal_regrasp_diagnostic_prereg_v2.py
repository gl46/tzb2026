#!/usr/bin/env python3
"""Build the fresh six-object ADR-0026 diagnostic V2 preregistration."""
...
LEDGER_ROOT = "/var/tmp/xh-data/isaac-industrial/m2c/s4-terminal-regrasp-diagnostic-ledger-v2"
TEMPLATE_PATH = "robot_ws/src/xh_sim/worlds/industrial_cylinder_v1.sdf"
CANDIDATE_BASE_SEED_START = 260100


class DiagnosticV2PreregBuildError(ValueError):
    """The frozen inputs cannot produce the V2 preregistration."""
...
def _git(project_root: Path, *args: str) -> bytes:
    ...
    if completed.returncode != 0:
        raise DiagnosticV2PreregBuildError(
...
def _json_object(payload: bytes, *, label: str) -> dict[str, Any]:
    try:
        ...
        raise DiagnosticV2PreregBuildError(f"{label} is not JSON") from error
    if not isinstance(value, dict):
        raise DiagnosticV2PreregBuildError(f"{label} is not a JSON object")
    return value
...
def _external_identities(
    *, project_root: Path, commit: str
    ...
    if any(not values for values in axes.values()):
        raise DiagnosticV2PreregBuildError("external identity inventory is incomplete")
...
def _select_runs(
    *,
    template: str,
    external: Mapping[str, set[Any]],
) -> list[dict[str, Any]]:
    runs: list[dict[str, Any]] = []
    selected_base_seeds = 0
    source_base_seed = CANDIDATE_BASE_SEED_START
    while selected_base_seeds < RUNS_PER_CONDITION:
        ...
        if full is not None:
            ...
            if not any(record[axis] in external[axis] for record in candidate for axis in external):
                ...
                selected_base_seeds += 1
        source_base_seed += 1
        if source_base_seed >= CANDIDATE_BASE_SEED_START + 10_000:
            raise DiagnosticV2PreregBuildError("cannot find twelve fresh base seeds")
    return runs
...
def build_prereg(*, project_root: Path, source_commit: str = "HEAD") -> dict[str, Any]:
    ...
    if _sha256(_git_blob(root, commit, ADR_PATH)) != ADR_SHA256:
        raise DiagnosticV2PreregBuildError("accepted ADR-0026 bytes changed")
    if _git_blob(root, ADR_INTRODUCED_COMMIT, ADR_PATH) != _git_blob(root, commit, ADR_PATH):
        raise DiagnosticV2PreregBuildError("accepted ADR-0026 binding changed")
    for path in IMPLEMENTATION_PATHS:
        current = (root / path).read_bytes()
        if current != _git_blob(root, commit, path):
            raise DiagnosticV2PreregBuildError(
                f"V2 implementation is not exact source commit: {path}"
            )
    ...
    v1_report = _json_object(
        _git_blob(root, commit, V1_INCOMPLETE_REPORT_PATH),
        label=V1_INCOMPLETE_REPORT_PATH,
    )
    if (
        v1_report.get("status") != "BLOCKED_INCOMPLETE_PRE_ACTION_SCHEMA_MISMATCH"
        or v1_report.get("valid_terminal_measurements") != 0
        or v1_report.get("failed_run", {}).get("probe_launched") is not False
        or v1_report.get("failed_run", {}).get("physical_action_outcome") != "NOT_AVAILABLE"
    ):
        raise DiagnosticV2PreregBuildError("V1 invalidation evidence changed")
    core: dict[str, Any] = {
        ...
        "v1_invalidation": {
            "preregistration": {
                "path": V1_PREREG_PATH,
                "sha256": _sha256(_git_blob(root, commit, V1_PREREG_PATH)),
            },
            "incomplete_audit": {
                "path": V1_INCOMPLETE_REPORT_PATH,
                "sha256": _sha256(_git_blob(root, commit, V1_INCOMPLETE_REPORT_PATH)),
            },
        ...
        "implementation_bindings": [
            {
                "path": path,
                "sha256": _sha256(_git_blob(root, commit, path)),
            }
            for path in IMPLEMENTATION_PATHS
        ],
    ...
    M2CTerminalRegraspDiagnosticPreregV2.model_validate(prereg)
    return prereg
...
def write_create_only(path: Path, payload: bytes) -> None:
    ...
    try:
        ...
        while offset < len(payload):
            ...
            if written <= 0:
                raise OSError("failed to publish V2 diagnostic preregistration")
            offset += written
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
