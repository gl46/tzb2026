#!/usr/bin/env python3
"""Replay the three consumed Batch-20 attempts without upgrading evidence."""
...
PREREG_RELATIVE = Path("docs/decisions/M2C-S4-V4-TRAIN-COLLECTION-BATCH-20-PREREG.json")
PREREG_FILE_SHA256 = "b4c195f9487cdbf9ceaaa8fdb9377d90e2082d7afb2691ef0a10c477c723c68c"
PREREG_SHA256 = "f1a8e92bbc205e2cf8da74bfaf19fc58912419f64a527101c4b8494f4ef15390"
PREREG_COMMIT = "c0a3358dc854ea537d549183c793c925d2664129"
SOURCE_SNAPSHOT_COMMIT = "a48cb6b84bdef03449dbac975dac3e9eb85cc433"
SOURCE_SNAPSHOT_INVENTORY_SHA256 = (
    "3e0e5624452b568bc74584977c9e8e2aa0bc281cf1a54f4b5f1ec0c14d01cb08"
)
PREDECESSOR_RECEIPT_SHA256 = "abd30cd8762c723a10afadfe15078c6e5074abaf9a8757e18b63d94bd9b0e81d"
CLAIM_FILE_SHA256 = (
    "c124c46b825cc1253858e2c9ac4993f85e067141f93e6b2a9ecbc7b657c7fe05",
    "8b44b5d52d956a1d5241e06a17f881796768ae47fb24cf9adb39333edceb112d",
    "c0fd42d7b1fa8b4e2f8e55c22b835fd9f7c19572c68e48d8984c11fddcb81e08",
)
...
class Batch20AuditError(Batch10AuditError):
    """Batch-20 bytes do not prove the bounded mixed zero-eligible outcome."""
...
def _validate_claim_and_job(
    *,
    ...
    try:
        ...
        claim = json_object(claim_raw, label=f"Batch-20 claim {ordinal}")
        job = json_object(read_regular_file_once(root / relative["job"]), label="Batch-20 job")
    except FileNotFoundError as error:
        raise Batch20AuditError("Batch-20 claim or job evidence is missing") from error
    if sha256_bytes(claim_raw) != CLAIM_FILE_SHA256[ordinal] or claim_raw != projected_raw:
        raise Batch20AuditError("Batch-20 canonical and projected claim bytes differ")
    if not _claim_core_is_valid(claim):
        raise Batch20AuditError("Batch-20 claim semantic receipt hash is invalid")
    if (
        claim.get("schema_version") != "M2CS4V4CollectionConsumptionReceiptV1"
        or claim.get("batch_id") != "m2c-s4-v4-train-batch-20"
        or claim.get("ledger_namespace") != "M2C_S4_V4_COLLECTION_BATCH20"
        ...
        raise Batch20AuditError("Batch-20 claim identity or hash chain differs")
    ...
    if (
        ...
        raise Batch20AuditError("Batch-20 job does not bind the consumed claim")
    if inventory.get(relative["derived_probe"]) != claim.get("derived_probe_sha256"):
        raise Batch20AuditError("Batch-20 derived probe hash differs from its claim")
...
def build_report(*, evidence_root: Path, project_root: Path = ROOT) -> dict[str, Any]:
    ...
    if sha256_bytes(prereg_raw) != PREREG_FILE_SHA256:
        raise Batch20AuditError("Batch-20 preregistration bytes changed")
    if (
        _git_blob(
        ...
        raise Batch20AuditError("Batch-20 preregistration differs from its introduction blob")
    prereg = json_object(prereg_raw, label="Batch-20 preregistration")
    ...
    if (
        ...
        raise Batch20AuditError("Batch-20 preregistration contract differs")
    inventory = _inventory(root)
    if len(inventory) != 118:
        raise Batch20AuditError("Batch-20 evidence inventory count differs")
    ...
    if job_keys != {str(item["matched_key"]) for item in selected}:
        raise Batch20AuditError("Batch-20 evidence job set differs from selected keys")
    ...
    for ordinal, identity in enumerate(selected):
        if not isinstance(identity, Mapping):
            raise Batch20AuditError("Batch-20 selected identity is malformed")
        ...
        expected_file_count = 5 if ordinal == 0 else 55
        if len(job_inventory) != expected_file_count:
            raise Batch20AuditError("Batch-20 per-job evidence inventory count differs")
        if ordinal == 0:
            ...
            try:
                outcome = _validate_physical_attempt(
                    ...
                    prefix=prefix,
                    relative=relative,
                    expected_detection_counts=EXPECTED_DETECTION_COUNTS[ordinal - 1],
                    expected_final_status=EXPECTED_FINAL_STATUSES[ordinal - 1],
                    training_manifest=training_manifest,
                    s6_manifest=s6_manifest,
                )
            except Batch10AuditError as error:
                raise Batch20AuditError(str(error)) from error
            if (
                tuple(outcome["offline_dataset_exclusion_reasons"])
                != EXPECTED_DATASET_EXCLUSIONS[ordinal - 1]
            ):
                raise Batch20AuditError("Batch-20 offline dataset exclusion reasons differ")
            raw_probe = json_object(
                read_regular_file_once(root / relative["raw_probe"]),
                label="Batch-20 raw probe",
            )
            current_receipt_ids = {
            ...
            if all_receipt_ids & current_receipt_ids:
                raise Batch20AuditError("Batch-20 physical receipt ID is reused")
            all_receipt_ids.update(current_receipt_ids)
            outcome.update(
                {
                    "classification": EXPECTED_CLASSIFICATIONS[ordinal - 1],
        ...
        attempts.append(
            {
                "attempt_id": f"batch20-{ordinal + 1:02d}",
                "identity": identity,
    ...
    if (root / "packaged").exists() and any((root / "packaged").rglob("*")):
        raise Batch20AuditError("Batch-20 evidence contains packaged output")
    ...
    return {
        ...
        "audit_implementation": {
            "path": "scripts/m2c/audit_s4_v4_batch20_collection.py",
...
def render_markdown(report: Mapping[str, Any]) -> str:
    return f"""# M2C S4 V4 Batch-20 collection audit

Status: `{report["status"]}`

The three outcome-blind preregistered extension TRAIN keys were consumed exactly once, with no
retry or replacement. Scene 22008 reached Isaac startup but the stage process exited with code
139 before stage acceptance. Scenes 22022 and 22025 completed strict eight-step V4 physical
...
- scene 22008: stage process exit 139; no probe and no physical action.
- scene 22022: steps 0-6 passed; REGRASP ended `CONTACT_GATE_REJECTED`; offline replay also
  records `STEP_1:PUBLIC_TARGET_OUTSIDE_V4_K8` and `STEP_2:PUBLIC_TARGET_OUTSIDE_V4_K8`.
- scene 22025: steps 0-6 passed; REGRASP ended `PREGRASP_IK_GATE_REJECTED`.
...
```bash
PYTHONPATH=src:scripts uv run python scripts/m2c/audit_s4_v4_batch20_collection.py \\
  --evidence-root /Users/gl/tzb-m2c-evidence/m2c-s4-v4-batch20-complete \\
  --expected-json reports/m2c-s4-v4-batch20-collection.json
```
"""
...
def main() -> int:
    ...
    if args.expected_json is not None and read_regular_file_once(args.expected_json) != encoded:
        raise SystemExit("Batch-20 audit differs from expected JSON")
    if args.markdown:
