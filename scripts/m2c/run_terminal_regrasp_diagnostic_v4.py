#!/usr/bin/env python3
"""Run exactly one consumed ADR-0026 V4 terminal diagnostic attempt."""
...
from m2c.run_terminal_regrasp_diagnostic_v3 import (
    _probe_command_v3 as _probe_command_v4,
)
...
def run_one_v4(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if len(selected) != 1:
        raise TerminalDiagnosticError("V4 run ID is not uniquely preregistered")
    run = selected[0]
    if args.image != ISAAC_IMAGE:
        raise TerminalDiagnosticError("V4 image tag differs from preregistration")
    ...
    if (
        ...
        raise TerminalDiagnosticError("V4 source root must be root-owned 0555 evidence")
    for path, expected, label in (
        ...
        if path.parent.resolve(strict=True) != args.source_root.resolve(strict=True):
            raise TerminalDiagnosticError(f"V4 {label} is not under source root")
        if _sha256_file(path) != expected:
            raise TerminalDiagnosticError(f"V4 {label} digest differs from prereg")
    if _sha256_file(args.upstream_v4_probe) != resolved.prereg.upstream_v4_probe_sha256:
        raise TerminalDiagnosticError("V4 upstream probe digest differs from freeze")
    derived_bytes = derive_terminal_regrasp_diagnostic_probe_bytes_v4(
    ...
    if image_id != ISAAC_IMAGE_ID:
        raise TerminalDiagnosticError("V4 image ID differs from preregistration")
    ...
    if (
        ...
        raise TerminalDiagnosticError("V4 output root must be root-owned 0700")
    job_root = args.output_root / (
        f"run-{run.ordinal:02d}-{run.run_id.removeprefix('m2c-s4-terminal-diagnostic-')[:16]}"
    )
    if job_root.exists():
        raise FileExistsError(f"refusing to overwrite V4 diagnostic job: {job_root}")
    ...
    derived_probe = derived_root / "terminal-regrasp-diagnostic-probe-v4.py"
    ...
    claim = consume_diagnostic_run_v4(
        resolved=resolved,
        run_id=run.run_id,
    ...
    job_receipt: dict[str, Any] = {
        "schema_version": "M2CTerminalDiagnosticJobReceiptV4",
        "status": "CONSUMED_V4_DIAGNOSTIC_ONLY_BEFORE_STAGE",
    ...
    try:
        ...
        if stage.returncode != 0 or not _stage_is_valid(
            ...
            raise TerminalDiagnosticError("V4 stage failed its source-hash gate")
        terminal["stage_completed"] = True
        for member in (
            stage_root / "metrics.json",
            stage_root / "m1b_physics_scene.usdc",
        ):
            member.chmod(0o400)
        probe_command = _probe_command_v4(
            image_id=image_id,
            gpu=args.gpu,
        ...
        if not raw_path.is_file():
            raise TerminalDiagnosticError("V4 probe emitted no terminal raw evidence")
        raw = parse_diagnostic_raw_bytes_v4(v4_auth.read_regular_file_once(raw_path))
        if raw.run_id != run.run_id:
            raise TerminalDiagnosticError("V4 raw evidence belongs to another run")
...
def main() -> int:
    ...
    parser.add_argument("--container-prefix", default="m2c-terminal-diagnostic-v4")
