...
failure_root="${M2B_FAILURE_ROOT:-/var/tmp/xh-data/isaac-industrial/m2b/failure-evidence-scale-v1}"
output_root="${M2B_OUTPUT_ROOT:-/var/tmp/xh-data/isaac-industrial/m2b/residual-evidence-scale-v1}"
...
if ssh -o BatchMode=yes "${remote_host}" \
  pgrep -f 'scripts/m2b/run_.*evidence_worker.py' >/dev/null; then
  echo "an M2B evidence worker is already active on ${remote_host}" >&2
  exit 3
fi
...
for gpu in 0 1; do
  ...
  remote_command=(
    python3 "${project_root}/scripts/m2b/run_residual_evidence_worker.py"
    --project-root "${project_root}"
    --source-root "${source_root}"
    --failure-worker-root "${failure_root}/worker${gpu}"
