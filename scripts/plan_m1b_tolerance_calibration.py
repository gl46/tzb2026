...
def main() -> int:
    ...
    instances=c.get("calibration_instances", [])
    if len(instances) != c["repetitions_per_point"]:
        raise SystemExit("calibration_instances must provide exactly one distinct instance per repetition")
    if len({(item.get("scene_seed"), item.get("object_slot")) for item in instances}) != len(instances):
        raise SystemExit("calibration_instances must be distinct scene/object pairs")
    for axis in c["offset_axes"]:
        for offset in c["offsets_m"]:
            for repetition in range(1,c["repetitions_per_point"]+1):
                ...
                trials.append({"axis":axis,"offset_m":offset,"repetition":repetition,"provenance":"CALIBRATION_ONLY_INITIALIZATION","required_distinct_instance_slot":repetition,"scene_seed":int(instance["scene_seed"]),"object_slot":int(instance["object_slot"])})
